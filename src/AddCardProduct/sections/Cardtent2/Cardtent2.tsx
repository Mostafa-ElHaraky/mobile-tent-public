...
interface Cardtent2Props {
  ...
  productType?: string; // Current product type for filtering other sizes
  
  // Contact info
  contacts?: ContactInfo;
  
  // Document files (could be passed from parent)
...
export const Cardtent2 = ({ 
  ...
  specs,
  priceConfigurations,
  otherSizes: propOtherSizes,
  productType = 'Арочные (Шатры)', // Default value
  contacts,
  documents: propDocuments,
  videos: propVideos
}: Cardtent2Props): ReactElement => {
  
  // ===== DEBUG: Log when component mounts and productType changes =====
  useEffect(() => {
    console.log('🔍 Cardtent2 - productType received:', productType);
  }, [productType]);

  // ===== STATE =====
  const [activeTabLabel, setActiveTabLabel] = useState<string>("Описание и характеристики");
  ...
  // ===== CONSTANTS (can be overridden by props) =====
  ...
  // Documents - can be passed from parent or use default
  const pdfFiles: Array<{ title: string; filename: string }> = propDocuments || [
  ...
  const documents: Array<{ title: string; filename: string }> = pdfFiles;

  // Videos - can be passed from parent or use default
  ...
  // Fetch other sizes products - USING DYNAMIC productType WITH DEBUG LOGS
  useEffect(() => {
    const fetchOtherSizes = async () => {
      try {
        ...
        const staticFiltered = allProducts.filter((p) => p.type === productType);
        console.log('🔄 Fallback products found:', staticFiltered.length, staticFiltered);
    ...
    fetchOtherSizes();
  }, [productType]); // only depend on productType

  // DEBUG: Log when otherSizes changes
  useEffect(() => {
    console.log('📦 otherSizes updated:', otherSizes.length, 'items for type:', productType);
  }, [otherSizes, productType]);
  ...
  const sizesNext = () => {
    if (otherSizes.length > 0) {
      setSizesMobileIndex((prev) => (prev + 1) % otherSizes.length);
    }
  };
  
  const sizesPrev = () => {
    if (otherSizes.length > 0) {
      setSizesMobileIndex((prev) => (prev - 1 + otherSizes.length) % otherSizes.length);
    }
  };
  ...
  const DocActions = ({ doc }: { doc: { title: string; filename: string } }) => {
    const fileUrl = `/files/${encodeURIComponent(doc.filename)}`;
    return (
  ...
  // Helper to render specs if available
  const renderSpecs = () => {
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        {specs.productCode && (
          <div className="text-sm">
            ...
            <span className="ml-2 font-semibold">{specs.productCode}</span>
          </div>
        )}
        {specs.size && (
          <div className="text-sm">
            <span className="text-gray-500">Размер:</span>
            <span className="ml-2 font-semibold">{specs.size}</span>
          </div>
        )}
        {specs.area && (
          <div className="text-sm">
            <span className="text-gray-500">Площадь:</span>
            <span className="ml-2 font-semibold">{specs.area} м²</span>
          </div>
        )}
        {specs.shape && (
          <div className="text-sm">
            <span className="text-gray-500">Форма:</span>
            <span className="ml-2 font-semibold">{specs.shape}</span>
          </div>
        )}
        {specs.capacity && (
          <div className="text-sm">
            <span className="text-gray-500">Вместимость:</span>
            <span className="ml-2 font-semibold">{specs.capacity}</span>
          </div>
        )}
        {specs.width && specs.length && (
          <div className="text-sm">
            <span className="text-gray-500">Ширина × Длина:</span>
            <span className="ml-2 font-semibold">{specs.width}м × {specs.length}м</span>
          </div>
        )}
        {specs.archHeight && (
          <div className="text-sm">
            <span className="text-gray-500">Высота арки:</span>
            <span className="ml-2 font-semibold">{specs.archHeight}м</span>
          </div>
        )}
        {specs.ridgeHeight && (
          <div className="text-sm">
            <span className="text-gray-500">Высота в коньке:</span>
            <span className="ml-2 font-semibold">{specs.ridgeHeight}м</span>
          </div>
        )}
        {specs.inStock && (
          <div className="text-sm">
            <span className="text-gray-500">Наличие:</span>
            <span className={`ml-2 font-semibold ${specs.inStock === 'Да' ? 'text-green-600' : 'text-orange-600'}`}>
              {specs.inStock}
            </span>
          </div>
        )}
      ...
      <div className="hidden md:block relative w-full max-w-[1440px] mx-auto bg-white">
        ...
        <div className="flex flex-col w-full">
          ...
          <SizeSelectionSection 
            ...
            specs={specs ? [
              { label: "Размер:", value: specs.size || "—" },
              { label: "Площадь:", value: specs.area ? `${specs.area} м²` : "—" },
              { label: "Форма:", value: specs.shape || "—" },
              { label: "Вместимость:", value: specs.capacity || "—" },
              { label: "Ширина:", value: specs.width ? `${specs.width} м` : "—" },
              { label: "Длина:", value: specs.length ? `${specs.length} м` : "—" },
              { label: "Высота входной арки:", value: specs.archHeight ? `${specs.archHeight} м` : "—" },
              { label: "Высота в коньке:", value: specs.ridgeHeight ? `${specs.ridgeHeight} м` : "—" },
            ] : undefined}
          />
          ...
          <div className="w-full px-12 mt-16">
            ...
            {loadingOtherSizes ? (
              ...
              <div className="text-center py-8 text-gray-600">
                Товары не найдены для типа: {productType}
              </div>
            )}
          </div>
        ...
        <div className="my-6">
          <SizeSelectionSection 
            ...
            specs={specs ? [
              ...
              { label: "Высота входной арки:", value: specs.archHeight ? `${specs.archHeight} м` : "—" },
              { label: "Высота в коньке:", value: specs.ridgeHeight ? `${specs.ridgeHeight} м` : "—" },
              { label: "В наличии:", value: specs.inStock || "—" },
            ] : undefined}
          />
        </div>
