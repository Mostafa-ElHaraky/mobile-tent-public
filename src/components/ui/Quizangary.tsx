...
const validRussianPhone = (input: string) => {
...
export const Quizangary = () => {
  ...
  const [isSending, setIsSending] = useState(false); // ← added, doesn’t remove anything
  
  const capacityOptions: CapacityOption[] = [
    { 
      value: "industry",  
      label: "Промышленность", 
      photo: "/tentovyequiz1.webp"
    },
    { 
      value: "agriculture",  
      label: "Сельское хозяйство", 
      photo: "/tentovyequiz2.webp"
    },
    { 
      value: "sport",  
      label: "Спорт", 
      photo: "/tentovyequiz3.webp"
    },
    { 
      value: "special-project",  
      label: "Спецпроект", 
      photo: "/tentovyequiz4.webp"
    },
    { 
      value: "custom-project",  
      label: "Нестандартный проект", 
      photo: "/tentovyequiz5.webp"
    },
    { 
      value: "not-sure", 
  ...
  // Ensure paths with spaces/parentheses are properly encoded in href
  ...
  const tentUsageOptions: TentUsageOption[] = [
    {
      ...
      label: "Торговля",
      image: {
        src: "/503774b4-f2ac-4d80-a16e-e0e416349220-------1.webp",
        width: 174,
        height: 174,
      }
    },
  ...
  // -------- updated to your preferred normalizer (useCallback) --------
const formatPhone = (value: string) => {
  // digits only
  const digits = value.replace(/\D/g, "").replace(/^8/, "7"); // if user types 8, replace with 7
  const onlyNums = digits.substring(0, 11); // max 11 digits for Russian numbers

  let result = "+7";
  if (onlyNums.length > 1) result += "(" + onlyNums.substring(1, 4);
  if (onlyNums.length >= 4) result += ")";
  if (onlyNums.length >= 4) result += "-" + onlyNums.substring(4, 7);
  if (onlyNums.length >= 7) result += "-" + onlyNums.substring(7, 9);
  if (onlyNums.length >= 9) result += "-" + onlyNums.substring(9, 11);
...
const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // -------- form metadata & direct webhook POST (added) --------
  const formData = {
    title:
  ...
  const sendToWebhook = async (cleanPhoneDigits: string) => {
    const now = new Date();
    ...
    const domainHost =
      typeof window !== "undefined" ? window.location.hostname : "tent-test.ru";
    ...
    const sourceDescription =
      `Клиент нажал «${formData.buttonText}» и оставил телефон. ` +
      `Форма: «${formData.title}». ` +
      `Страница: ${pageUrl} | Время клиента: ${clientTime} (${tz}) | ` +
      `Дата создания (клиента): ${clientDate} | Источник: сайт ${domainHost} (форма контактов).`;
    ...
    const payload: Record<string, string> = {
      ...
      Источник: `Сайт ${domainHost} — форма «${formData.buttonText}»`,
    ...
    try {
      await fetch(
        "https://crm.grand-tent.ru/local/webhooks/get_from_sites.php",
        {
          ...
          body,
          mode: "no-cors", // keep if CORS is not enabled on the CRM endpoint
        }
      );
    } catch (e) {
  ...
  // -------- updated to call webhook before redirect (no removals) --------
  const handleSubmit = async () => {
    // keep your existing validation rules
    if (!validateForm()) return;

    const clean = sanitizeInput(phone);
    if (!validRussianPhone(clean)) {
    ...
    try {
      setIsSending(true);
      const digits = clean.replace(/\D/g, ""); // "7XXXXXXXXXX"
      await sendToWebhook(digits);
    } finally {
      setIsSending(false);
    }
...
const renderCapacityOptions = () => (
          ...
          <CardContent className="p-0 h-full">
            <div className="p-5 h-full flex flex-col justify-between">
              {option.icon ? (
                ...
                // Фото, если задано
                <div className={`${isMobile ? 'mt-2 ml-[10px]' : 'mt-5 ml-[22px]'} relative w-[calc(100%-22px)] ${isMobile ? 'h-[80px]' : 'h-[110px]'}`}>
                  <Image
                    src={option.photo}
                    alt={option.label}
                    fill
                    className="object-contain"
                    sizes="(max-width: 768px) 90vw, 318px"
                    loading="lazy"
                    quality={90}
                    priority={false}
                  />
                ...
                // Текстовое представление, если фото нет
                <div className={`${isMobile ? 'mt-2 ml-[10px]' : 'mt-5 ml-[22px]'} font-bold ${isMobile ? 'text-4xl' : 'text-6xl'} text-[#6FA7FF]`}>
                  {option.display}
                </div>
              )}
  ...
  const renderPricingTiers = () => (
    <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-3'} gap-2 w-full relative left-[550px]`}>
      {pricingTiers.map((tier, index) => (
        <div 
          key={index} 
          className={index === 2 ? (isMobile ? "col-span-1" : "md:col-span-2 flex justify-center") : "col-span-1"}
        >
          <Card className={`${isMobile ? 'w-full' : 'w-[450px]'} border border-solid border-[#dddddd] rounded-[20px] shadow-[0px_24px_38px_0px_rgba(22,29,36,0.08)] overflow-hidden`}>
        ...
        <div className="w-full flex items-center gap-4">
          <div className="w-[75px] h-[70px] flex-shrink-0 flex items-center justify-center"> {/* جعل نفس حجم العنصر الأول */}
            <Image
              ...
              alt="Img"
              src="/Fc3740fa54440e5eaefe94690ddc2643-4.webp"
              width={50}
              height={50}
              loading="lazy"
            />
          ...
          <p className="font-normal text-white text-base">
            Примеры уже готовых ангаров схожими
            <br />с вашими задачами
          </p>
        </div>
      </div>
<div className="absolute bottom-[120px] right-[-20px] w-[180px] h-[180px]"> {/* تم التكبير من 150x150 إلى 180x180 */}
  <Image
  ...
  />
    ...
    return (
      <div className="absolute w-[594px] h-[377px] bottom-[400px] right-0">
          ...
          <div className="absolute top-[96px] left-[30px] flex flex-col items-start gap-5">
            <div className="relative w-[378px] h-14">
              ...
              <p className="absolute top-4 left-[86px] font-normal text-white text-base">
                3d проект вашего ангара под размеры
                <br />и оснащение
              </p>
            </div>
              ...
              <>
                ...
                <div className="flex flex-col items-start gap-3 w-full p-6 bg-[#f2f4f7] rounded-[10px] mb-6">
                  <p className="font-semibold text-[#4f5d80] text-base">
                    Получите расчет цены ангара в 3х вариантах ответив на 3 вопроса
                  </p>
                  <Progress
...
case 2:
  return (
    <div className="flex flex-col items-start gap-6 mb-12 relative">
      ...
      <h2 className="font-bold text-[#394355] text-xl">
        2. Где будет использоваться ангар?
      </h2>
      ...
      {/* Add mt-8 or similar margin-top class for desktop */}
      {!isMobile && <div className="mt-[150px]">{renderGiftsSection()}</div>}
    </div>
  );
  ...
  return (
    <div className="relative w-full max-w-[1366px] mx-auto">
      <div className="w-full">
        <div className="w-full">
          ...
          <div className={`flex flex-col ${isMobile ? 'w-full' : 'w-[627px]'} items-start gap-6 ${isMobile ? 'pt-[20px]' : 'pt-[52px]'}`}>
            ...
            <div className="flex flex-col items-start gap-6 mt-[30px]">
              <h2 className="font-bold text-[#394355] text-xl">
                3. В какой срок необходимо установить ангар?
              </h2>
              <RadioGroup 
        ...
        {/* Move the gifts section down for desktop */}
        {!isMobile && (
          <div className="mt-32"> {/* Add top margin to push it down */}
            {renderGiftsSection()}
          </div>
        )}
        ...
        return (
          <div className="w-full relative">
            <div className="flex flex-col items-start gap-6 mb-6">
              <h2 className="font-bold text-[#394355] text-xl">
                4. Предварительный расчет стоимости ангаров
              </h2>
                  ...
                  <div className="flex flex-col items-start gap-[45px]">
                    <div className={`flex flex-col w-full ${isMobile ? '' : 'max-w-[544px]'} items-start gap-6`}>
                      <h3 className="font-semibold text-[#394355] text-2xl">
                        Для точного расчета стоимости ангара нужно получить больше данных
                      </h3>
                    ...
                    <div className="flex flex-col items-start gap-[31px]">
                      <div className="flex flex-col items-start justify-end gap-4 w-full relative z-10">
                        <div className="flex w-full h-[67px] items-start rounded-2xl overflow-hidden border border-solid border-neutral-200 relaive z-10">
                            <Input
                              value={phone}
                      ...
                      <div className="flex w-full max-w-[544px] items-center gap-2.5 relative z-20">
                        <button 
                        ...
                        <p className="font-medium text-[#4f5d8094] text-xs">
                          ...
                          <a href="/privacy-policy" target="_blank" className="text-[#3c6cec] underline ml-1">
...
<DialogTrigger asChild>
  <Button
    type="button" // ✅ prevents form submit default
    onClick={(e) => {
      e.preventDefault();     // ✅ block any default anchor/submit behavior
      e.stopPropagation();    // ✅ don't bubble to a parent Card/Link
      setOpen(true);
    }}
            ...
            <Card key={doc.path} className="border border-[#e5e7eb]">
              <CardContent className="p-4 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {/* Force download (same-origin + download attr) */}
    ...
    {isMobile ? (
      ...
      <div className="flex flex-col gap-9">
        <div className="flex flex-col gap-6">
          ...
          <p className="font-['Raleway',Helvetica] font-medium text-[#4f5d80] text-lg leading-6">
            А это на минуточку 50% от цены всего ангара
          </p>
        </div>
