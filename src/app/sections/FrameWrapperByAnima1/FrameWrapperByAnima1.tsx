...
export const FrameWrapperByAnima1 = (): ReactElement => {
    ...
    <>
      ...
      <section className="hidden md:flex flex-col items-start gap-9 w-full max-w-[1344px] mx-auto my-12">
        ...
        <div className="flex items-start gap-6 w-full">
          {desktopItems.map((item) => (
            <Card
              key={item.type}
              className={`w-full max-w-[660px] h-[660px] rounded-[20px] border-0 ${item.isDark
              ...
              <CardContent className="p-0">
                <div className="p-10">
                  <div className="flex flex-col items-start gap-6">
                    <h3
                      className={`font-bold text-2xl leading-6 whitespace-nowrap font-['Raleway',Helvetica] ${item.isDark ? "text-[#394355]" : "text-white"
                        }`}
                    >
                      {item.title}
                    </h3>
                    <p
                      className={`font-semibold text-lg leading-6 font-['Raleway',Helvetica] ${item.isDark ? "text-[#394355]" : "text-white"
                        }`}
                    >
                      {item.subtitle}
                    </p>
                    <p
                      className={`font-medium text-base leading-6 font-['Raleway',Helvetica] ${item.isDark ? "text-[#394355]" : "text-white"
                        }`}
                    >
                      {item.description}
                    </p>
                ...
                <div className="relative w-[580px] h-[389px] mx-auto mt-[-10px] rounded-3xl overflow-hidden">
                  <Image
                    ...
                    fill
                    className={`rounded-3xl object-cover ${item.isDark ? "grayscale brightness-75" : ""
                      }`}
                    loading="lazy"
                  />
                </div>
      ...
      <section className="md:hidden flex flex-col items-start gap-6 w-full px-4 my-8">
        ...
        <div className="flex flex-col items-center gap-4 w-full">
          <Card
            key={mobileItem.type}
            className={`w-full h-auto rounded-[20px] border-0 ${mobileItem.isDark
                ? "bg-white border border-solid border-[#cecece] shadow-[var(--)] backdrop-blur-[79px]"
                : "bg-[#6FA7FF] shadow-2"
              }`}
          >
            <CardContent className="p-6">
              <div className="flex flex-col items-start gap-4">
                <h3
                  className={`font-bold text-xl leading-6 font-['Raleway',Helvetica] ${mobileItem.isDark ? "text-[#394355]" : "text-white"
                    }`}
                >
                  {mobileItem.title}
                </h3>
                <p
                  className={`font-semibold text-base leading-6 font-['Raleway',Helvetica] ${mobileItem.isDark ? "text-[#394355]" : "text-white"
                    }`}
                >
                  {mobileItem.subtitle}
                </p>
                <p
                  className={`font-medium text-sm leading-6 font-['Raleway',Helvetica] ${mobileItem.isDark ? "text-[#394355]" : "text-white"
                    }`}
                >
              ...
              <div className="relative w-full h-[240px] mt-4 rounded-xl overflow-hidden">
                <Image
                  ...
                  fill
                  className={`rounded-xl object-cover ${mobileItem.isDark ? "grayscale brightness-75" : ""
                    }`}
                  loading="lazy"
                />
              </div>
