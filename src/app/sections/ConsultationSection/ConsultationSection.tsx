...
export const ConsultationSection = (): ReactElement => {
  ...
  return (
    <>
      {/* Desktop Version */}
      <section className="hidden desktop:block relative w-[1440px] h-[726px] rounded-[30px] overflow-hidden shadow-[var(--)] backdrop-blur-[79px] backdrop-brightness-[100%] [-webkit-backdrop-filter:blur(79px)_brightness(100%)] mx-auto flex justify-center items-center">
      ...
      <section className="block desktop:hidden relative w-full py-12 md:py-24 px-6 md:px-12 bg-gradient-to-b from-[#f5f6ff] to-transparent max-w-6xl mx-auto">
        <div className="relative">
          <h2 className="text-3xl md:text-5xl font-bold text-center mb-10 md:mb-16 [font-family:'Raleway',Helvetica] leading-tight max-w-4xl mx-auto">
            <span className="text-[#527dc5] block md:inline">Получите экспертную консультацию </span>
            <span className="text-[#232c42] block md:inline">по покупке шатра с оценкой стоимости и сроков</span>
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12 md:mb-20">
            {benefits.map((benefit) => (
              <div key={benefit.id} className="flex items-start gap-4 p-4 md:p-6 bg-white/50 rounded-2xl backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow">
                <div className="relative w-8 h-8 md:w-10 md:h-10 mt-1 flex-shrink-0">
                  <Image loading="lazy" className="w-full h-auto" alt="Check icon" src={benefit.icon} width={40} height={28} />
                </div>
                <div className="flex-1 font-medium text-[#394355] text-base md:text-lg leading-relaxed [font-family:'Raleway',Helvetica]">
                  {benefit.text}
                </div>
              </div>
            ))}
          ...
          <div className="w-full mb-16 md:mb-24 max-w-2xl mx-auto bg-white p-6 md:p-10 rounded-3xl shadow-xl border border-gray-100">
            <div className="flex flex-col gap-6">
              <div className="flex flex-col md:flex-row items-end gap-4">
                <Card className="w-full md:flex-1 border-0 shadow-none">
                  <CardContent className="p-0 border-0">
                    <div className="flex items-center gap-3 pl-4 pr-3 py-3 md:py-4 bg-gray-50 rounded-2xl border border-gray-200 focus-within:border-[#527dc5] transition-colors">
                      <Input
                        className="border-none shadow-none w-full bg-transparent [font-family:'Raleway',Helvetica] font-semibold text-[#232c42] text-lg md:text-xl"
                        placeholder="+7 (XXX) XXX-XX-XX"
                        value={phone}
                        onChange={handlePhoneChange}
                        onKeyDown={handlePhoneKeyDown}
                ...
                <Button
                  className="w-full md:w-auto h-[60px] md:h-[72px] px-8 rounded-2xl [background:linear-gradient(180deg,rgba(115,168,255,1)_0%,rgba(111,167,255,1)_37%,rgba(60,108,236,1)_100%)] font-bold text-white text-lg [font-family:'Raleway',Helvetica] shadow-lg hover:scale-[1.02] transition-transform"
                  onClick={handleSubmit}
                  disabled={isSending}
                >
                  {isSending ? 'Отправка...' : 'Получить консультацию'}
              ...
              <div className="flex items-start justify-center gap-3">
                <div className="flex items-center h-5 mt-1">
                  <input
                    ...
                    checked={isChecked}
                    onChange={handleCheckboxChange}
                    className="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 rounded-md focus:ring-blue-500 focus:ring-2 cursor-pointer"
                  />
                </div>
                <label htmlFor="privacy-checkbox-mobile" className="cursor-pointer">
                  <p className="font-medium text-[#4f5d8094] text-xs md:text-sm [font-family:'Raleway',Helvetica] text-left opacity-80">
                    Я даю согласие на обработку{" "}
                    <a href="/privacy-consent" className="text-[#73A8FF] font-bold hover:underline">персональных данных</a>{" "}
                    и согласен с{" "}
                    <a href="/privacy-policy" className="text-[#73A8FF] font-bold hover:underline">политикой конфиденциальности</a>
          ...
          <div className="relative w-full flex flex-col md:flex-row items-center justify-center gap-12 max-w-5xl mx-auto">
            <div className="relative w-full max-w-md aspect-[4/5] rounded-[32px] overflow-hidden shadow-2xl md:order-2">
              <Image loading="lazy" className="object-cover" alt="Event tent" src="/element8-event-tent-i02-1.webp" fill />
              <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
            </div>
            
            <div className="w-full max-w-md md:order-1">
              <div className="relative flex flex-col items-center">
                <div className="w-full rounded-[32px] overflow-hidden shadow-2xl">
                  <Image loading="lazy" className="w-full h-auto" alt="Confident young businessman" src="/element8-confident-young-businessman-in-blue-shirt-and-glasses-smiling-at.webp" width={516} height={718} />
                </div>
                <Card className="absolute -bottom-8 md:-right-8 w-full max-w-[320px] [background:linear-gradient(159deg,rgba(36,48,87,1)_0%,rgba(55,66,85,1)_100%)] border-none shadow-2xl p-2 rounded-2xl">
                  <CardContent className="p-4 md:p-6">
                    <div className="flex flex-col items-center justify-center gap-2 text-center">
                      <div className="font-bold text-white text-lg md:text-xl [font-family:'Raleway',Helvetica]">Евгений Петров</div>
                      <div className="font-medium text-white/90 text-sm md:text-base [font-family:'Raleway',Helvetica]">Эксперт по тентовым конструкциям</div>
                    </div>
                  </CardContent>
                </Card>
              </div>
          ...
          <Image loading="lazy" className="absolute bottom-[-100px] left-[-50px] w-1/4 -z-10 opacity-20 blur-sm" alt="Vecteezy overgrown" src="/element8-vecteezy-overgrown-green-grass-generative-ai-generated-24190129--1.webp" width={302} height={322} />
          <Image loading="lazy" className="absolute bottom-[-80px] right-[-50px] w-1/4 -z-10 opacity-20 blur-sm" alt="Vecteezy overgrown" src="/element8-vecteezy-overgrown-green-grass-generative-ai-generated-24190129--2.webp" width={359} height={306} />
        </div>
      </section>
    </>
  );
