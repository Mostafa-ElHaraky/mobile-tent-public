...
interface FrameMainOurProjects4Props {
  // SEO data
  seoData?: {
    title?: string;
    description?: string;
...
export const FrameMainOurProjects4 = ({ 
  seoData,
  contacts,
  content 
}: FrameMainOurProjects4Props): ReactElement => {
  // 👉 Images for the carousel - use from CMS or default
  ...
  const carouselImages = content?.carouselImages || defaultCarouselImages;
  ...
  // Generate contact information from CMS data
  const generateContactInfo = (): ContactInfoItem[] => {
    ...
    // Override with CMS data if available
    if (contacts) {
      return defaultContactInfo.map(item => {
        switch(item.type) {
          case 'address':
            return contacts.office?.address 
              ? { ...item, text: contacts.office.address } 
              : item;
          
          case 'hours':
            return contacts.office?.hours 
              ? { 
                  ...item, 
                  text: contacts.office.hours,
                  subText: contacts.office.appointment || item.subText
                } 
              : item;
          
          case 'phone':
            if (item.text.includes('113-36-60') && contacts.phone1) {
              return { ...item, text: contacts.phone1, phoneLink: `tel:${contacts.phone1.replace(/\D/g, '')}` };
            }
            if (item.text.includes('302-46-31') && contacts.phone2) {
              return { ...item, text: contacts.phone2, phoneLink: `tel:${contacts.phone2.replace(/\D/g, '')}` };
            }
            return item;
          
          case 'email':
            return contacts.email 
              ? { ...item, text: contacts.email, mailLink: `mailto:${contacts.email}` } 
              : item;
  ...
  // Generate benefits from CMS data or use defaults
  const defaultBenefits: BenefitItem[] = [
    {
      icon: "/element8-2.webp",
      title: "Посмотрите и потрогаете ткань вживую",
      description: "Оцените состояние металла каркаса на реальных объектах после нескольких лет воздействия осадков",
    },
    {
  ...
  const telegramLink = contacts?.telegram || "https://t.me/+79770893996?start=14594990928";
  const whatsappLink = contacts?.whatsapp || "https://api.whatsapp.com/send/?phone=79805109568";
        ...
        <div className="max-w-[1466px] mx-auto relative">
          ...
          <div className="font-normal text-4xl leading-normal font-['Raleway',Helvetica] tracking-[0] mb-10">
            <span className="font-semibold text-[#232c42]">
              {content?.heading || "Весь комплекс работ делаем на своем производстве "}
            </span>
            <span className="font-bold text-[#527dc5]">
              {content?.subheading || "в г. Калуга"}
            </span>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="flex flex-col gap-9 w-full md:w-1/2">
              ...
              <div className="flex items-center gap-4 p-3 bg-[#f2f4f7] rounded-[10px]">
                ...
                <div className="font-medium text-[#4f5d80] text-lg leading-6 font-['Raleway',Helvetica] tracking-[0]">
                  {content?.invitationText || "Приезжайте на знакомство-экскурсию"}
                </div>
              </div>
            ...
            <div className="w-full md:w-1/2">
              ...
              <div className="relative flex mb-8">
                ...
                {/* Right image (next) */}
                <div
                  className="relative w-[328px] h-[310px] bg-cover bg-[50%_50%]"
                  style={{
