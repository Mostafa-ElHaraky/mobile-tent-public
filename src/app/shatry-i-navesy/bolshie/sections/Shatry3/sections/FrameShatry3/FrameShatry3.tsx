...
export const FrameShatry3 = (): ReactElement => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        console.log('Fetching Большие шатры with updated prices...');
        
        // Use the shared utility to get products with updated prices
        const products = await getProductsWithUpdatedPrices();
        
        // Filter only Большие шатры products
        const filteredProducts = products.filter((p) => p.type === 'Большие шатры');
        
        console.log(`Loaded ${filteredProducts.length} Большие шатры products`);
        
        // If no products from CMS, use fallback static data
        if (filteredProducts.length === 0) {
          console.warn('No Большие шатры products from CMS, using fallback data');
          // Filter allProducts for fallback
          const fallbackProducts = allProducts.filter((p) => p.type === 'Большие шатры');
          setArochnyeShatry(fallbackProducts);
        } else {
          setArochnyeShatry(filteredProducts);
        }
      } catch (error) {
        console.error('Error fetching Большие шатры products:', error);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Большие шатры completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Большие шатры failed:', error);
      }
      ...
      <section className="mx-auto max-w-[1440px] w-full relative bottom-[5px] px-4 md:px-8">
        ...
        {arochnyeShatry.length === 0 && !isLoading ? (
          <div className="text-center py-10">
            <p className="text-gray-500">Нет доступных Большие шатров в данный момент</p>
          </div>
        ) : (
          <>
