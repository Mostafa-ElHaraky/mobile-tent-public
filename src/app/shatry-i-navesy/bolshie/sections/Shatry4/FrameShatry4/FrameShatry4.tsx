...
export const FrameShatry4 = ({ 
        ...
        <section className="hidden md:block relative bottom-[90px] w-full">
          <div className="mx-auto max-w-[1440px] flex flex-col items-start gap-9 px-10">
            <h2 className="w-fit font-semibold text-4xl [font-family:'Raleway',Helvetica] leading-tight">
              <span className="text-[#527dc5]">Просторные шатры</span>{" "}
              <span className="text-[#232c42]">для мероприятий от Мобайл-Тент</span>
            </h2>
  ...
  return (
    <>
      ...
      <section className="hidden md:block relative bottom-[90px] w-full">
        <div className="mx-auto max-w-[1440px] flex flex-col items-start gap-9 px-10">
          <h2 className="w-fit font-semibold text-4xl [font-family:'Raleway',Helvetica] leading-tight text-[#232c42]">
            Просторные шатры для мероприятий от <span className="text-[#527dc5]">Мобайл-Тент</span> — надёжное решение любого масштаба
          </h2>
          
          {/* Dynamic content from CMS - ALL HTML WILL COME FROM HERE */}
          {cleanedDesktopContent ? (
            <div 
              className="w-full font-medium text-lg leading-7 text-[#394355] space-y-6 [&_ul]:list-disc [&_ul]:pl-6 [&_li]:mb-1 [&_h3]:font-semibold [&_h3]:text-2xl [&_h3]:mt-8 [&_h3]:text-[#232c42] [&_b]:font-semibold [&_b]:text-[#232c42]"
              dangerouslySetInnerHTML={parseHtmlContent(cleanedDesktopContent)}
              suppressHydrationWarning
            />
          ) : (
            /* EMPTY FALLBACK - NO DEFAULT CONTENT, ALL FROM CMS */
            <div className="w-full font-medium text-lg leading-7 text-[#394355] space-y-6">
              {/* No default content - waiting for CMS data */}
            </div>
          )}
        </div>
      </section>
      ...
      <section className="block md:hidden px-4 py-6 bg-white rounded-lg shadow-sm mx-4 my-4">
        <div className="flex flex-col gap-4">
          <h2 className="text-lg font-semibold text-center leading-tight text-[#232c42]">
            Большие шатры
          </h2>

          {/* Mobile dynamic content from CMS */}
