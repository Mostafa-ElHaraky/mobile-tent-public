...
export const FrameShatry3 = (): ReactElement => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        console.log('Fetching Звезда Шатры with updated prices...');
        
        // Use the shared utility to get products with updated prices
        const products = await getProductsWithUpdatedPrices();
        
        // Filter only Звезда products
        const filteredProducts = products.filter((p) => p.type === 'Звезда');
        
        console.log(`Loaded ${filteredProducts.length} Звезда products`);
        
        // If no products from CMS, use fallback static data
        if (filteredProducts.length === 0) {
          console.warn('No Звезда products from CMS, using fallback data');
          // Filter allProducts for fallback
          const fallbackProducts = allProducts.filter((p) => p.type === 'Звезда');
          setArochnyeShatry(fallbackProducts);
        } else {
          setArochnyeShatry(filteredProducts);
        }
      } catch (error) {
        console.error('Error fetching Звезда products:', error);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Звезда completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Звезда failed:', error);
      }
