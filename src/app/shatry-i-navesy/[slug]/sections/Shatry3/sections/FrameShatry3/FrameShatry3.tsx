...
interface FrameShatry3Props {
  productType: string;      // e.g., "Арочные (Шатры)"
  sectionTitle: string;     // e.g., "Арочные шатры"
}

export const FrameShatry3 = ({ productType, sectionTitle }: FrameShatry3Props): ReactElement => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const allProductsData = await getProductsWithUpdatedPrices();
        const filteredProducts = allProductsData.filter((p) => p.type === productType);
        
        console.log(`Loaded ${filteredProducts.length} ${productType} products`);
        
        if (filteredProducts.length === 0) {
          console.warn(`No ${productType} products from CMS, using fallback data`);
          const fallbackProducts = allProducts.filter((p) => p.type === productType);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setProducts(filteredProducts);
          console.log(`Scheduled price update for ${productType} completed`);
        }
      } catch (error) {
        console.error(`Scheduled price update for ${productType} failed:`, error);
      }
    }, 10 * 60 * 1000);
    
    return () => clearInterval(interval);
  }, [productType]); // Re-run when productType changes
  ...
  useEffect(() => {
    setVisibleCount(4); // Reset to show only first 4 products
  }, [products]);
      ...
      <section className="mx-auto max-w-[1440px] w-full relative bottom-[5px] px-4 md:px-8">
        ...
        {products.length === 0 && !isLoading ? (
          <div className="text-center py-10">
            <p className="text-gray-500">Нет доступных товаров в данный момент</p>
          </div>
        ) : (
          <>
