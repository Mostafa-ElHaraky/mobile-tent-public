...
export const FrameShatry3 = (): ReactElement => {
  ...
  // Control for showing more products
  const [visibleCount, setVisibleCount] = useState<number>(4); // Default show 4 products
  const PRODUCTS_PER_PAGE = 4; // Number of products to add each time
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        console.log('Fetching Арочные (Шатры) Шатры with updated prices...');
        
        const products = await getProductsWithUpdatedPrices();
        const filteredProducts = products.filter((p) => p.type === 'Арочные (Шатры)');
        
        console.log(`Loaded ${filteredProducts.length} Арочные (Шатры) products`);
        
        if (filteredProducts.length === 0) {
          console.warn('No Арочные (Шатры) products from CMS, using fallback data');
          const fallbackProducts = allProducts.filter((p) => p.type === 'Арочные (Шатры)');
          setArochnyeShatry(fallbackProducts);
        } else {
          setArochnyeShatry(filteredProducts);
        }
      } catch (error) {
        console.error('Error fetching Арочные (Шатры) products:', error);
        const fallbackProducts = allProducts.filter((p) => p.type === 'Арочные (Шатры)');
        setArochnyeShatry(fallbackProducts);
      } finally {
        setIsLoading(false);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Арочные (Шатры) completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Арочные (Шатры) failed:', error);
      }
    }, 10 * 60 * 1000);
  ...
  useEffect(() => {
    setVisibleCount(6); // Reset to show only first 4 products
  }, [arochnyeShatry]);
  ...
  const hasMoreToShow = visibleCount < arochnyeShatry.length;
  
  const handleShowMore = () => {
    setVisibleCount(prev => Math.min(prev + PRODUCTS_PER_PAGE, arochnyeShatry.length));
  };
      ...
      <section className="mx-auto max-w-[1440px] w-full relative bottom-[5px] px-4 md:px-8">
        ...
        {arochnyeShatry.length === 0 && !isLoading ? (
          <div className="text-center py-10">
            <p className="text-gray-500">Нет доступных арочных шатров в данный момент</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3 gap-6">
              {arochnyeShatry.slice(0, visibleCount).map((product) => {
            ...
            {/* Show More Button - Shows when there are more products to display */}
            {hasMoreToShow && (
              <div className="mt-8 flex justify-center">
                <button
                  onClick={handleShowMore}
                  className="w-full max-w-[300px] h-[48px] rounded-2xl bg-gradient-to-b from-[#73A8FF] via-[#6FA7FF] to-[#3C6CEC] text-white text-sm font-semibold shadow-[0px_12px_24px_0px_rgba(32,124,254,0.4)] hover:brightness-110 transition-all duration-200"
                  type="button"
                >
                  Показать ещё ({arochnyeShatry.length - visibleCount})
                </button>
              </div>
            )}
          </>
