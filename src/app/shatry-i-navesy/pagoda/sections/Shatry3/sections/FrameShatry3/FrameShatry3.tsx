...
export const FrameShatry3 = (): ReactElement => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        console.log('Fetching Пагода Шатры with updated prices...');
        ...
        const filteredProducts = products.filter((p) => p.type === 'Пагода');
        
        console.log(`Loaded ${filteredProducts.length} Пагода Шатры products`);
        ...
        if (filteredProducts.length === 0) {
          ...
          const fallbackProducts = allProducts.filter((p) => p.type === 'Пагода');
          setArochnyeShatry(fallbackProducts);
        } else {
          setArochnyeShatry(filteredProducts);
        }
      } catch (error) {
        console.error('Error fetching Пагода Шатры products:', error);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Пагода Шатры completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Пагода Шатры failed:', error);
      }
