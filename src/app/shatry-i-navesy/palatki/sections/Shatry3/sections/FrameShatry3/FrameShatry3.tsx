...
import { allProducts } from '@/data/hitSalesData';
...
export const FrameShatry3 = (): ReactElement => {
  ...
  // NEW: Mobile "show more" control (kept from your last version)
  const [visibleProducts, setVisibleProducts] = useState<number>(0);

  const arochnyeShatry = useMemo(
    () => allProducts.filter((p) => p.type === 'Шатры для выставок'),
    []
  );
  ...
  return (
    <>
      {/* Desktop Version */}
      <section className="relative mx-auto max-w-[1440px] h-[334px] bottom-[30px] rounded-[30px] bg-gradient-to-br from-[#243057] to-[#374255] flex hidden md:flex overflow-hidden">
      ...
      {/* Product Section — Restyled cards to match your reference */}
      <section className="mx-auto max-w-[1440px] w-full mt-10 px-4 md:px-8">
