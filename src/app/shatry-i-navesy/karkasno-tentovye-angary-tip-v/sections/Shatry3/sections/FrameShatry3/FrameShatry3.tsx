...
export const FrameShatry3 = (): ReactElement => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        console.log('Fetching Ангары Тип В with updated prices...');
        
        // Use the shared utility to get products with updated prices
        const products = await getProductsWithUpdatedPrices();
        
        // Filter only Ангары Тип В products
        const filteredProducts = products.filter((p) => p.type === 'Ангары Тип В');
        
        console.log(`Loaded ${filteredProducts.length} Ангары Тип В products`);
        
        // If no products from CMS, use fallback static data
        if (filteredProducts.length === 0) {
          console.warn('No Ангары Тип В products from CMS, using fallback data');
          // Filter allProducts for fallback
          const fallbackProducts = allProducts.filter((p) => p.type === 'Ангары Тип В');
          setArochnyeShatry(fallbackProducts);
        } else {
          setArochnyeShatry(filteredProducts);
        }
      } catch (error) {
        console.error('Error fetching Ангары Тип В products:', error);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Ангары Тип В completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Ангары Тип В failed:', error);
      }
      ...
      <section className="mx-auto max-w-[1440px] w-full relative bottom-[5px] px-4 md:px-8">
        ...
        {arochnyeShatry.length === 0 && !isLoading ? (
          <div className="text-center py-10">
            <p className="text-gray-500">Нет доступных Ангары Тип В Ангары в данный момент</p>
          </div>
        ) : (
          <>
