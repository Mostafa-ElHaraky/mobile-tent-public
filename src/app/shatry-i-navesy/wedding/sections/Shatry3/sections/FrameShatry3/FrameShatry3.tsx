...
export const FrameShatry3 = (): ReactElement => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        console.log('Fetching Свадьбы шатры Шатры with updated prices...');
        
        // Use the shared utility to get products with updated prices
        const products = await getProductsWithUpdatedPrices();
        
        // Filter only Свадьбы шатры products
        const filteredProducts = products.filter((p) => p.type === 'Свадьбы шатры');
        
        console.log(`Loaded ${filteredProducts.length} Свадьбы шатры products`);
        
        // If no products from CMS, use fallback static data
        if (filteredProducts.length === 0) {
          console.warn('No Свадьбы шатры products from CMS, using fallback data');
          // Filter allProducts for fallback
          const fallbackProducts = allProducts.filter((p) => p.type === 'Свадьбы шатры');
          setArochnyeShatry(fallbackProducts);
        } else {
          setArochnyeShatry(filteredProducts);
        }
      } catch (error) {
        console.error('Error fetching Свадьбы шатры products:', error);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Свадьбы шатры completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Свадьбы шатры failed:', error);
      }
