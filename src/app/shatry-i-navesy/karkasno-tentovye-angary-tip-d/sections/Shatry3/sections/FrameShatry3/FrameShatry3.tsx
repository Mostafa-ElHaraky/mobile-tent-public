...
export const FrameShatry3 = (): ReactElement => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        console.log('Fetching Ангары Тип Д with updated prices...');
        
        // Use the shared utility to get products with updated prices
        const products = await getProductsWithUpdatedPrices();
        
        // Filter only Ангары Тип Д products
        const filteredProducts = products.filter((p) => p.type === 'Ангары Тип Д');
        
        console.log(`Loaded ${filteredProducts.length} Ангары Тип Д products`);
        
        // If no products from CMS, use fallback static data
        if (filteredProducts.length === 0) {
          console.warn('No Ангары Тип Д products from CMS, using fallback data');
          // Filter allProducts for fallback
          const fallbackProducts = allProducts.filter((p) => p.type === 'Ангары Тип Д');
          setArochnyeShatry(fallbackProducts);
        } else {
          setArochnyeShatry(filteredProducts);
        }
      } catch (error) {
        console.error('Error fetching Ангары Тип Д products:', error);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Ангары Тип Д completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Ангары Тип Д failed:', error);
      }
      ...
      <section className="mx-auto max-w-[1440px] w-full relative bottom-[5px] px-4 md:px-8">
        ...
        {arochnyeShatry.length === 0 && !isLoading ? (
          <div className="text-center py-10">
            <p className="text-gray-500">Нет доступных Ангары Тип Д Ангары в данный момент</p>
          </div>
        ) : (
          <>
        ...
        <div className="flex-1 py-16 px-[85px]">
          <div className="flex flex-col gap-6 mb-[46px]">
            <h2 className="w-full font-['Raleway',Helvetica] text-4xl tracking-[0]">
              ...
              <span className="font-bold text-[#9ec2ff]">ангар </span>
            ...
            <p className="w-full font-['Raleway',Helvetica] font-medium text-white text-lg tracking-[0] leading-6">
              Получите каталог ангары и посмотрите в удобное для вас время
            </p>
          </div>
