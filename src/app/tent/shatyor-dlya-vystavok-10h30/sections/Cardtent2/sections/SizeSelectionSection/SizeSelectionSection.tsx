...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для выставок 10×30
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для выставок 10х30 м",
      text: `Участие в выставке или организация презентации — это значительные траты, но провести мероприятие на высоком уровне и оптимизировать бюджет возможно, если взять в аренду один или несколько профессиональных шатров размером 10×30 м.

Выставку под открытым небом важно защитить от непогоды. Навесная конструкция обеспечит высокий уровень комфорта, а приобретённый или арендованный шатёр можно использовать многократно.`,
    },
    {
      title: "Конструкция и материалы",
      text: `• Не требует специального фундамента, устанавливается на любой земляной поверхности.
• Монтируется и демонтируется без применения спецтехники в минимальные сроки, без потери качества, любое количество раз.
...
• Особенности монтажа: обязательное крепление конструкции к поверхности установки или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `Огромная площадь — 300 м². Пространство надёжно защищено от ветра и атмосферных осадков.
Транспортировка на место не доставит проблем: шатёр легко устанавливать и демонтировать.
Для дополнительного комфорта гостей можно доукомплектовать конструкцию опциями — установить калориферы, освещение, мебель, напольное покрытие.`,
    },
    {
      title: "Особенности монтажа",
      text: `Шатёр для выставок 10×30 м устанавливается без фундамента; для устойчивости крепления применяются утяжелители.
Допускается многократный монтаж/демонтаж — это не ухудшает внешний вид конструкции; срок службы достигает 10 лет.
Для установки не требуется спецтехника.
Выгоднее всего заказывать шатры напрямую у производителя. Компания — надёжный поставщик навесных конструкций: гарантируем высокое качество и доступные цены.`,
    },
  ];
