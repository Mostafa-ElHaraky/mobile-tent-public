...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (20×20 — 400 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Воздушный купол огромного шатра на 400 м² сразу притягивает к себе взгляды. Он станет лучшей площадкой для организации самых ярких и престижных ивентов. Арендовать или купить этот шатёр выгодно, ведь он дает полную свободу в выборе места проведения праздника и помогает экономить средства на аренде помещений.

Наше предприятие является производителем, благодаря прямым продажам у нас нет огромных посреднических наценок. Работая с нами, Вы сможете приобрести недорого шатры и тенты на условиях оптовой и розничной торговли.

Монтаж больших шатров — процесс сложный и занимающий немало времени. В некоторых случаях для установки конструкции необходимо задействовать спецтехнику.`,
    },
  ];

  // Default specs from your original code (20×20 — 400 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "4 100 кг" },
    { label: "Нагрузка на опорную пятку", value: "512 кг" },
