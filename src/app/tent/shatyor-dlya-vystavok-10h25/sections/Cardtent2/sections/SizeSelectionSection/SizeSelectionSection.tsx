...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для выставок 10×25
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для выставок 10х25 м",
      text: `Найти пространство, цена которого не ударит по бюджету выставки или презентации, вполне реально. 
Просторный шатёр для выставок 10х25 м даст свободу в выборе места проведения и позволит воплотить в жизнь любую идею декора.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности установки или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `Прочный и лёгкий каркас выполнен из анодированного алюминия; ПВХ не гниёт и не горит.
Площадь конструкции — 250 м²; при необходимости её увеличивают, присоединяя другие тенты.
После приобретения шатра в собственность его можно устанавливать неограниченное количество раз — конструкция быстро окупается.`,
    },
    {
      title: "Особенности монтажа",
      text: `На место проведения конструкция доставляется в сложенном виде — транспортировка удобна.
Быстрая установка на любой ровной поверхности (для комфорта гостей можно постелить ковролин или обустроить пол).
Шатёр 10х25 м спроектирован с учётом удобства монтажа: крепления надёжные, для установки не требуется спецтехника.
MOBILE TENT — крупнейший российский производитель навесных конструкций. Поможем приобрести или арендовать шатёр или тент для любых задач по доступной цене.`,
    },
  ];
