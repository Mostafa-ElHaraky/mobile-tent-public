...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (8×8 — 64 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание модели",
      text: `Выполненный из светопрозрачного полотна шатёр, купить который по доступной цене Вы можете у нас, имеет широкую сферу применения. 
Легкую в процессе монтажа и демонтажа конструкцию вполне возможно использовать в ходе организации выездной торговли или для предоставления кейтеринговых услуг.`,
    },
    {
      title: "Особенности конструкции",
      text: `Эргономичный дизайн быстровозводимого арочного шатра вполне соответствует современным модным тенденциям. 
Приобрести вместительный, но невероятно компактный внешне шатёр недорого Вы можете у нас в любое удобное время! 

При производстве арочного шатра 8×8 м используются только высококачественные, соответствующие общепринятым стандартам материалы и комплектующие, а это значит, что конструкция прослужит Вам длительное время, без потерь в эстетических и эксплуатационных характеристиках!`,
    },
  ];

  // Default specs from your original code (8×8 — 64 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Высота в коньке", value: "2,87 м" },
    { label: "Вес конструкции", value: "810 кг" },
    { label: "Нагрузка на опорную пятку", value: "202 кг" },
