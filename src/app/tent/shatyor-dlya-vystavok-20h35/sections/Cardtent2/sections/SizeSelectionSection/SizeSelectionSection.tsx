...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для выставок 20х35
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для выставок 20х35 м",
      text: `У Вас намечается выставка, а Вы не можете подобрать подходящее помещение? Предлагаем отличное решение проблемы — шатёр для выставок 20х35 м. Конструкция имеет современный дизайн и способна создать уютные условия для гостей.

Российская компания производит шатровые конструкции любых размеров и конфигураций. Специалисты нашего предприятия спроектируют и изготовят шатёр для Вашего мероприятия в самые короткие сроки. Мы проводим последующий ремонт и полное обслуживание конструкции.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности, на которую ставится, или утяжеление конструкции бетонными блоками.`,
    },
    {
      ...
      text: `• Надёжное основание и прочное ПВХ, которое может быть выполнено в любом цвете.
• Наличие оконных проёмов способствует большему комфорту. Их количество может варьироваться в зависимости от пожеланий заказчика.
• Тентовая конструкция имеет современный дизайн. У нас можно заказать дополнительные аксессуары и необходимое оборудование (кондиционеры, напольное покрытие, мебель и т.д.).`,
    },
    {
      title: "Особенности монтажа",
      text: `• Доставка и монтаж шатра для выставок размером 20х35 м не требует привлечения специальной техники.
• Конструкция быстро устанавливается на ровную поверхность. После монтажа обязательно следует закрепить и утяжелить каркас.

Мы оперативно выполним доставку и монтаж заказанной Вами тентовой конструкции в указанном месте.`,
    },
  ];

  // TECH SPECS — UPDATED for 20х35
  const specs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "10 500 кг" },
