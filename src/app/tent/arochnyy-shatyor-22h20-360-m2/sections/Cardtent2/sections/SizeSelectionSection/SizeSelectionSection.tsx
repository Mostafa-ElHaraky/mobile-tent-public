...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your code (22×20 — 360 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Особенности шатра",
      text: `Арочный шатёр размером 22×20 м и площадью 360 м² вместит 225 человек при банкетной рассадке и 325 — при проведении фуршета. Вместительный шатёр можно установить на любой ровной поверхности. Оцинкованная сталь, используемая для изготовления каркаса и арок, делает конструкцию прочной и устойчивой при ветровой нагрузке.
...
Шатёр укомплектован белым тентом для стен и крыши из ПВХ. Существует возможность стыковать модуль с конструкцией, имеющей ширину арки в 10 метров. При проведении праздничного мероприятия стены шатра можно использовать для декорирования.`,
  ...
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    },
    {
      label: "Модульность",
      value:
        "Возможность увеличения площади путём соединения нескольких конструкций со стороной 5 м.",
    },
    {
