...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your code (20×20 — 400 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Классический шатёр 20х20 м квадратной формы — это мобильная конструкция, которая прекрасно подойдет для использования в качестве летнего кафе или для выездного мероприятия. Никакие порывы ветра или дождь не смогут испортить Вам праздник.`,
    },
    {
      title: "Характеристики и преимущества",
      text: `• Форма — квадратная, площадь — 400 м² (длина 20 м, ширина 20 м).
• Высота стены (опоры) — 3/3,5/4/4,5/5 м.
• Высота в коньке — макс. 8,5 м.
• Ветровая нагрузка — 30 м/сек.
• Нагрузка подвеса на арку — до 30 кг/п.м.
• Основной рамный профиль — 240 x100 x 5,00 сертификат QUALANOD.
• Дополнительный профиль — 166 x 89 x 4,00 сертификат QUALANOD.
• Профиль прогонов — 60 x 60 x 4,00 сертификат QUALANOD.`,
    },
    {
...
Особенности монтажа: обязательное крепление конструкции к поверхности, на которую ставится, или утяжеление конструкции бетонными блоками.`,
  ...
  // Default specs from your code (20×20 — 400 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Основной рамный профиль", value: "240 x100 x 5,00 сертификат QUALANOD" },
    ...
    },
    {
      ...
      value:
        "Сертифицированное ПВХ-полотно с добавками, препятствующими горению DIN 4102 B1, M20,3 кН / м ², гниению.",
    },
    {
      label: "Каркас",
      value:
