...
const PAGE_CODE = "glemping-diametr-6-m"; 
...
async function getPageData() {
  try {
    ...
    return {
      ...
      page: {
        id: 16,
        name: "Глэмпинг диаметр 6 м",
        code: PAGE_CODE
      },
      seo: {
        title: "Глэмпинг диаметр 6 м – купить в Москве с доставкой: цена в интернет-магазине Mobile Tent",
        description: "Глэмпинг диаметр 6 м – купить в Москве под заказ в компании Mobile Tent. Вы можете выбрать любую комплектацию, размер и цвет изделия, а также заказать его брендирование. Мы используем только качественные и долговечные материалы, а также предлагаем адекватные цены, которые будут дешевле долгосрочный аренды! Гарантия от производителя — 5 лет.",
        keywords: "",
        h1: "Глэмпинг диаметр 6 м",
        url: `/glemping-diametr-6-m`
      },
      content: {
        desktop: '',
        mobile: ''
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const seoTitle = data.seo?.title || "Глэмпинг диаметр 6 м – купить в Москве с доставкой: цена в интернет-магазине Mobile Tent";
  const seoDescription = data.seo?.description || "Глэмпинг диаметр 6 м – купить в Москве под заказ в компании Mobile Tent. Вы можете выбрать любую комплектацию, размер и цвет изделия, а также заказать его брендирование. Мы используем только качественные и долговечные материалы, а также предлагаем адекватные цены, которые будут дешевле долгосрочный аренды! Гарантия от производителя — 5 лет.";
  const seoKeywords = data.seo?.keywords || "";
  const seoUrl = data.seo?.url || `/tent/glemping-diametr-6-m`;
  ...
  return (
    <div className="flex flex-col w-full overflow-x-hidden">
      ...
      <h1 className="sr-only">
        {pageData.seo?.h1 || "Глэмпинг диаметр 6 м"}
      </h1>
