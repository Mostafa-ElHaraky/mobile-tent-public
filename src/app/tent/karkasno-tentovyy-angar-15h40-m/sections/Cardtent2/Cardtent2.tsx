...
import { Quizangary } from "../../../../../components/ui/Quizangary";
...
const pdfFiles: DocItem[] = [
  ...
  { title: "Ангары Тип Д конструкции. Гарантийная карта Бородина", filename: "Ангары Тип Д конструкции. Гарантийная карта Бородина.pdf" },
  ...
  useEffect(() => {
    const fetchOtherSizes = async () => {
      try {
        ...
        const products = await getProductsByType('Ангары Тип Д');
        setOtherSizes(products);
      } catch (error) {
        console.error('Error fetching other sizes:', error);
        // Fallback to static filtering
        const staticFiltered = allProducts.filter((p) => p.type === 'Ангары Тип Д');
