...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (8×8 — 64 м² variant)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Применение",
      text: `В тёплое время года благоприятная погода и красота природы способствуют организации праздников на открытом воздухе. 
Для таких целей арочный шатёр размером 8×8 м — это оптимальное решение с точки зрения цены и практичности. 

Талантливые сотрудники компании MOBILE TENT творчески подошли к решению задачи и спроектировали красивый и функциональный арочный шатёр. 
На его большой площади 64 м² комфортно будут себя чувствовать гости. 
Благодаря максимальной открытости конструкции, дневной свет хорошо освещает пространство.`,
    },
    {
      title: "Особенности монтажа",
      text: `Для тех, кто взял в аренду или решил купить большой арочный шатёр, важно помнить, что установку такой конструкции важно доверять только профессионалам. 
Для устойчивости и безопасности монтажа арочного шатра его необходимо правильно собрать и надёжно закрепить на поверхности.`,
    },
  ];

  // Default specs from your original code (8×8 — 64 м² variant)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Высота в коньке", value: "4,4 м" },
    { label: "Вес конструкции", value: "990 кг" },
    { label: "Нагрузка на опорную пятку", value: "248 кг" },
