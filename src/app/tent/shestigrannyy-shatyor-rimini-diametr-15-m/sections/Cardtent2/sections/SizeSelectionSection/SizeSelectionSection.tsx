...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шестигранный шатёр Римини (Диаметр 15 м)
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title:
        "Шестигранный шатёр Римини (Диаметр 15 м) — универсальное решение для мероприятий",
      text: `Полная свобода в выборе места для проведения праздника возможна, если у Вас есть шестигранный шатёр «Римини» диаметром 15 м. При площади 176 м² он остаётся сбалансированным по пропорциям и занимает немного места благодаря многоугольной форме, обеспечивая гостям достаточное свободное пространство.`,
    },
    {
      title: "Преимущества и характеристики",
      text: `• Покрытие выполнено из полотна ПВХ: современный материал устойчив к гниению и горению, не пропускает влагу.
• Шестигранный шатёр «Римини» диаметром 15 м освещается дневным светом — на каждой стене есть вставка из прозрачного материала.
• Вес — 971 кг; шатёр транспортируется в разобранном виде, доставка на место установки не вызывает сложностей.
• Технические параметры: самая высокая точка купола — 7,5 м; высота стен/опор — 2,5; 3; 3,5; 4 м; устойчивый к ветровой нагрузке (30 м/сек) каркас; нагрузка подвеса на арку — до 30 кг/п.м.
• Крепежи — оцинкованная сталь; каркас — анодированный алюминий. Металл лёгкий, устойчив к коррозии и пригоден к эксплуатации в любых погодных условиях.`,
    },
    {
...
• Крепёжные элементы — оцинкованная сталь DIN 50976.`,
    },
    {
      ...
      text: `• Фундамент не требуется: конструкция устанавливается на любой земляной поверхности или асфальте.
...
• Обязательное крепление конструкции к поверхности, на которую она устанавливается, или утяжеление бетонными блоками.`,
    },
  ];

  // TECH SPECS — UPDATED for Шестигранный шатёр Римини (Диаметр 15 м)
  const specs: { label: string; value: string }[] = [
    { label: "Модель", value: "Шестигранный шатёр Римини (Диаметр 15 м)" },
