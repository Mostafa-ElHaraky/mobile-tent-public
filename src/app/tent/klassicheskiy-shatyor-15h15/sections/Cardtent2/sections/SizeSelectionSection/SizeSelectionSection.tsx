...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING CONTENT — UPDATED for Классический шатёр 15х15
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Классический шатёр 15х15",
      text: `Для проведения мероприятия на улице Вы можете заказать классический шатёр размером 15х15 м, который можно недорого купить или взять в аренду у нас.`,
    },
    {
      title: "Преимущества и характеристики",
      text: `Шатёр 15х15 м в классическом стиле имеет квадратную форму. Общая площадь конструкции составляет 225 м². В таком шатре вполне можно расположить необходимое количество приглашённых.
Каркас изготовлен из алюминиевого профиля, а крепёжные элементы — из оцинкованной стали.
Тентовое полотно достаточно плотное и не теряет своих качественных характеристик после многократного монтажа шатра.
В собранном виде конструкция весьма компактна, несмотря на свой вес (1238 кг). Шатер удобен для транспортировки.`,
    },
    {
      title: "Особенности монтажа",
      text: `Наша компания предоставляет услуги монтажа заказанного шатра в случае необходимости.

Каждая конструкция имеет инструкцию по сборке, поэтому клиент может установить шатер собственными силами:
• монтаж производится на ровную поверхность;
• обязательно следует надёжно зафиксировать каркас и утяжелить основания.

Купить классический шатёр 15х15 м или оформить заявку на его аренду Вы можете прямо сейчас на нашем сайте.`,
    },
  ];

  // TECH SPECS — UPDATED with new data for 15x15m tent
  const specs: { label: string; value: string }[] = [
    ...
    { label: "Установка", value: "Не требует спец. фундамента, на любой земляной поверхности" },
    { label: "Монтаж/демонтаж", value: "Без спецтехники, быстрый монтаж и демонтаж без потери качества" },
