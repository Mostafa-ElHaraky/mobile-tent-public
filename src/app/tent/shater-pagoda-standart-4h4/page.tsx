...
const PAGE_CODE = "shater-pagoda-standart-4h4";
...
async function getPageData(): Promise<PageData> {
  try {
    ...
    // Fallback data for Pagoda tent Standard 4×4 m
    return {
      success: false,
      page: {
        id: 267,
        name: "Шатер Пагода Стандарт 4×4 м (16 м²)",
        code: PAGE_CODE,
      },
      seo: {
        title: "Шатер Пагода Стандарт 4×4 м (16 м²) – купить пагодный шатер в Москве | MOBILE TENT",
        description:
          "Шатер Пагода Стандарт 4×4 метра (площадь 16 м²) от производителя MOBILE TENT. Универсальная пагодная конструкция для торговли, мероприятий, кафе, складов. Алюминиевый каркас, ПВХ тент 650 г/м². Цена от 68 000 ₽. Гарантия 5 лет. Доставка по Москве, МО и всей России.",
        keywords:
          "шатер пагода 4х4, пагода шатер 4 на 4, шатер пагода стандарт, пагодный шатер 4х4, торговый шатер 4х4, шатер для мероприятий 4х4, шатер кафе 4х4, быстровозводимый шатер",
        h1: "Шатер Пагода Стандарт 4×4 м – универсальное решение для бизнеса",
        url: `/tent/${PAGE_CODE}`,
      },
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = data.seo?.title || "Шатер Пагода Стандарт 4×4 м (16 м²) – купить пагодный шатер в Москве | MOBILE TENT";
  const description =
    data.seo?.description ||
    "Шатер Пагода Стандарт 4×4 метра (площадь 16 м²) от производителя MOBILE TENT. Универсальная пагодная конструкция для торговли, мероприятий, кафе, складов. Алюминиевый каркас, ПВХ тент 650 г/м². Цена от 68 000 ₽. Гарантия 5 лет. Доставка по Москве, МО и всей России.";
  const keywords =
    data.seo?.keywords ||
    "шатер пагода 4х4, пагода шатер 4 на 4, шатер пагода стандарт, пагодный шатер 4х4, торговый шатер 4х4";
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: "Шатер Пагода Стандарт 4×4 метра",
        },
      ],
    },
    robots: {
...
export default async function PagodaTentPage() {
  ...
  const prices = pageData.priceConfigurations?.map((c) => c.currentPrice) || [68000, 98000, 135000];
  ...
  const productSchema = {
    ...
    "@type": "Product",
    image: "https://mobile-tent.ru/pagda4.webp",
    name: pageData.page?.name || "Шатер Пагода Стандарт 4×4 м (16 м²)",
    description:
      pageData.seo?.description ||
      "Шатер Пагода Стандарт 4×4 метра – универсальная пагодная конструкция для торговли, мероприятий, кафе, складов. Просторное внутреннее пространство, высота 3.5 м. Алюминиевый каркас, ПВХ тент 650 г/м², ветроустойчивость до 80 км/ч.",
    sku: `MOB-${PAGE_CODE}`,
    mpn: `MOB-PAGODA-4x4`,
    ...
    additionalProperty: [
      ...
      { "@type": "PropertyValue", name: "Высота в коньке", value: `${height} м` },
      { "@type": "PropertyValue", name: "Тип конструкции", value: "Пагодный шатер (Pagoda)" },
    ...
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: "Комплектации пагодного шатра 4×4 м",
      itemListElement:
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: pageData.page?.name || "Шатер Пагода Стандарт 4×4 м",
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "Какую площадь имеет шатер Пагода 4×4 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Площадь шатра составляет 16 м², высота в коньке – 3.5 метра. Это позволяет комфортно разместить до 10-12 человек за столами или до 15-18 человек на фуршет.",
        },
      },
      {
        "@type": "Question",
        name: "Сколько стоит шатер Пагода 4×4 м под ключ?",
        acceptedAnswer: {
          "@type": "Answer",
          text: `Цена от ${prices[0]?.toLocaleString() || "68 000"} ₽ за базовую комплектацию, утеплённая – от ${prices[1]?.toLocaleString() || "98 000"} ₽, максимальная (стационар) – от ${prices[2]?.toLocaleString() || "135 000"} ₽. Включает каркас, тент, боковые стенки, дверь. Доставка рассчитывается отдельно.`,
        },
      },
      {
        "@type": "Question",
        name: "Нужен ли фундамент для шатра Пагода 4×4 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Нет, капитальный фундамент не требуется. Шатер устанавливается на ровную площадку (грунт, асфальт, бетон, бетонные блоки) с фиксацией анкерами или утяжелителями.",
        },
      },
      {
        "@type": "Question",
        name: "Можно ли использовать шатер Пагода 4×4 м зимой?",
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: "Доставляете ли шатры Пагода 4×4 м в регионы России?",
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: "Для каких целей подходит шатер Пагода 4×4 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Идеален для: торговых точек, летних кафе, выездных мероприятий, свадеб, дней рождения, корпоративов, выставок, складов, временных офисов.",
        },
      },
      {
        "@type": "Question",
  ...
  return (
    <div className="flex flex-col w-full overflow-x-hidden">
      ...
      <h1 className="sr-only">{pageData.seo?.h1 || "Шатер Пагода Стандарт 4×4 м – универсальное решение для бизнеса"}</h1>
