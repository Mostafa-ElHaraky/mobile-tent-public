...
export const SizeSelectionSection = ({ 
  ...
  // MARKETING / DESCRIPTION — UPDATED for Тент большой 20х30
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Тент большой 20х30 — классическое решение для мероприятий",
      text: `Если Вы запланировали мероприятие на природе, то отличным решением будет аренда или покупка шатра 20х30 м, в котором свободно разместятся все приглашенные гости.`,
    },
    {
      title: "Преимущества конструкции",
      text: `Классический шатёр 20х30 м представляет собой прямоугольную тентовую конструкцию.
Основа выполнена из анодированного алюминия, крепёжные элементы — из оцинкованной стали. Общая площадь шатра — 600 м².
Тент пропитан огнеупорным составом и может быть выполнен в любом цвете.
Шатёр отлично подойдёт для проведения спортивных и культурных мероприятий, для деловой конференции или может быть использован в качестве торгового павильона под открытым небом.`,
    },
    {
      title: "Особенности монтажа",
      text: `В разобранном виде конструкция имеет компактные размеры и транспортируется обычным грузовым транспортом.
В инструкции есть указания по монтажу, благодаря которым Вы можете самостоятельно справиться с монтажными работами.
Шатёр устанавливается на ровную поверхность и обязательно надёжно крепится к ней. Следует дополнительно утяжелить каркас с помощью бетонных блоков.`,
    },
  ];

  // TECH SPECS — UPDATED for Тент большой 20х30
  const specs: { label: string; value: string }[] = [
    { label: "Модель", value: "Тент большой 20х30" },
