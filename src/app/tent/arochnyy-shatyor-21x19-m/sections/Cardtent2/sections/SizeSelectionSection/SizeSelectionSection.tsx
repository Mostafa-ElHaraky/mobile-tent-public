...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your code (21×19 — 305 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Иногда крупный шатёр может стать отличным решением для проведения мероприятий на открытом воздухе — под ним смогут укрыться от дождя или зноя до двухсот человек, при этом чувствуя себя комфортно. Идеально подойдут для таких целей арочные шатры размером 21×19 м.

Покупка такого крупного сооружения — удачный, но довольно затратный вариант. Поэтому для единичных мероприятий организаторы предпочитают брать тент в аренду. Несмотря на его огромную площадь, вы можете не беспокоиться о возможных проблемах при установке — монтаж не займёт много времени, особенно если проводить его будут специалисты компании Mobile Tent!`,
    },
    {
      title: "Применение",
      text: `Где применяется шатёр площадью 305 м²?
- Организация загородных корпоративных мероприятий для крупных компаний.
- Проведение спортивных мероприятий или концертов, на которых планируется установка большого количества посадочных мест.
- Для проведения фуршетов, крупных свадеб, выставок или ярмарок, а также многого другого.

Чтобы узнать подробнее о наших шатрах и условиях предоставления их в аренду, звоните по номеру, указанному на сайте. Наши менеджеры помогут вам подобрать подходящий вариант и ответят на любые вопросы.`,
    },
  ];

  // Default specs from your code (21×19 — 305 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Восьмигранник" },
    { label: "Площадь", value: "305 м²" },
    { label: "Длина", value: "21 м" },
    { label: "Ширина", value: "19 м" },
    { label: "Высота входной арки", value: "3,6 м" },
    { label: "Высота в коньке", value: "6,5 м" },
    { label: "Вес конструкции", value: "6500 кг" },
    { label: "Нагрузка на опорную пятку", value: "587 кг" },
    ...
    },
    {
      label: "Модульность",
      value:
        'Возможность увеличения площади путём соединения нескольких конструкций со стороной 8 м: "VIP-модуль Дюна 8×8 м", "VIP-модуль 16×16 м", "VIP-модуль Гексагональ 13×15 м".',
    },
    {
