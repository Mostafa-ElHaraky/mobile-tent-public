...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для мероприятий 20×35 м
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для мероприятий 20×35 м",
      text: `Сделав выбор в пользу шатра для мероприятий 20×35 м, вы будете уверены, что никакие погодные сложности не испортят ваш праздник. 
Конструкция отлично защищает от сильного ветра, дождя и солнца.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности, на которую ставится, или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `Шатёр для мероприятий 20×35 м общей площадью 700 м² прекрасно вместит большое количество гостей.
Прочный каркас. Тент не выгорает на солнце и может быть выполнен в любом цвете.
По желанию заказчика мы можем нанести на тент логотип компании или другое изображение любой площади.
Количество оконных проёмов может меняться в зависимости от особенностей мероприятия.`,
    },
    {
      title: "Особенности монтажа",
      text: `Шатровая конструкция устанавливается на ровную поверхность и надёжно крепится к ней.
Для доставки шатра не требуется специальный транспорт, поскольку в собранном виде конструкция компактна.
Специалисты нашей компании выполнят доставку и установку заказанного вами шатра в короткие сроки.`,
    },
  ];

  // TECH SPECS — UPDATED for 20×35
