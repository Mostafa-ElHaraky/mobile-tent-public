...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Тентовый ангар под мероприятие 15×20
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Тентовый ангар под мероприятие 15×20 — 300 м² функционального пространства",
      text: `Многие предприниматели всё чаще выбирают тентовые ангары: это дешевле и быстрее, чем строительство капитального объекта, при схожих эксплуатационных характеристиках.

Наше предприятие много лет занимается проектированием, изготовлением, ремонтом и комплексным обслуживанием каркасно-тентовых конструкций. Тентовый ангар 15×20 м — надёжное, выгодное и удобное решение. Мы выполняем все работы: от проектирования до сдачи «под ключ».`,
    },
    {
      title: "Преимущества и характеристики",
      text: `Прямоугольный тентовый ангар 15×20 м — вместительная конструкция общей площадью 300 м²:

• Прочный тент покрыт огнеупорной смесью, не выгорает на солнце и не теряет характеристик после многократного монтажа.  
• Возможность брендирования тента: нанесение логотипов и изображений.  
• Отлично выдерживает снеговые и ветровые нагрузки.  
• Площадь легко увеличить, соединив несколько конструкций между собой.`,
    },
    {
      title: "Особенности монтажа",
      text: `Ангар устанавливается на ровную поверхность и не требует фундамента.  
Монтаж и демонтаж выполняются без применения спецтехники в минимальные сроки.  
Закажите тентовый ангар 15×20 м прямо сейчас — позвоните или напишите нам!`,
    },
  ];

  // TECH SPECS — UPDATED for Тентовый ангар 15×20
  const specs: { label: string; value: string }[] = [
    { label: "Модель", value: "Тентовый ангар 15×20 (под мероприятие)" },
    ...
    { label: "Вес конструкции", value: "1 650 кг" },
