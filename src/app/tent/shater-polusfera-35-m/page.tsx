...
const PAGE_CODE = "shater-polusfera-35-m";
...
async function getPageData(): Promise<PageData> {
  try {
    ...
    // Fallback data for spherical tent diameter 35 m
    return {
      success: false,
      page: {
        id: 266,
        name: "Сферический шатёр диаметр 35 м (≈962 м²)",
        code: PAGE_CODE,
      },
      seo: {
        title: "Сферический шатёр диаметр 35 м (962 м²) – купить купольный шатер в Москве | MOBILE TENT",
        description:
          "Сферический шатёр диаметром 35 метров (площадь ≈962 м²) от производителя MOBILE TENT. Грандиозная купольная конструкция для концертов, выставок, спортивных мероприятий, авиаангаров. Алюминиевый каркас, ПВХ тент 900 г/м². Цена от 5 200 000 ₽. Гарантия 5 лет. Доставка по Москве, МО и всей России.",
        keywords:
          "сферический шатёр 35 м, купольный шатер 35 метров, шатер сфера 962 м², геодезический купол 35 м, шатер для концертов, ангар для авиации, выставочный купол",
        h1: "Сферический шатёр диаметром 35 метров – космическое пространство для грандиозных задач",
        url: `/tent/${PAGE_CODE}`,
      },
      ...
      priceConfigurations: [
        {
          ...
          discount: 16,
          description:
            "Алюминиевый каркас, ПВХ тент 650 г/м², стандартные двери, вентиляция. Для крупных летних мероприятий, выставок, складов.",
        },
        {
          name: "Оптимальная (утеплённая)",
          isActive: false,
        ...
        {
          name: "Максимальная (концертный зал / ангар)",
          isActive: false,
          currentPrice: 9500000,
          originalPrice: 11500000,
          discount: 17,
          description:
            "Стальной каркас с антикоррозийным покрытием, сэндвич-панели, освещение, отопление, кондиционирование, акустическая обработка, автоматические ворота.",
        },
      ],
      contacts: {
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = data.seo?.title || "Сферический шатёр диаметр 35 м (962 м²) – купить купольный шатер в Москве | MOBILE TENT";
  const description =
    data.seo?.description ||
    "Сферический шатёр диаметром 35 метров (площадь ≈962 м²) от производителя MOBILE TENT. Грандиозная купольная конструкция для концертов, выставок, спортивных мероприятий, авиаангаров. Алюминиевый каркас, ПВХ тент 900 г/м². Цена от 5 200 000 ₽. Гарантия 5 лет. Доставка по Москве, МО и всей России.";
  const keywords =
    data.seo?.keywords ||
    "сферический шатёр 35 м, купольный шатер 35 метров, шатер сфера 962 м², геодезический купол 35 м";
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: "Сферический шатёр диаметр 35 метров",
        },
      ],
    },
    robots: {
...
export default async function SphereTentPage() {
  ...
  const prices = pageData.priceConfigurations?.map((c) => c.currentPrice) || [5200000, 7200000, 9500000];
  const lowPrice = Math.min(...prices);
  const highPrice = Math.max(...prices);
  const offerCount = pageData.priceConfigurations?.length || 3;
  const diameter = 35;
  const area = 962; // π × (17.5²) ≈ 962
  const height = 35;
  ...
  const productSchema = {
    ...
    "@type": "Product",
    image: "https://mobile-tent.ru/spher26,1.webp",
    name: pageData.page?.name || "Сферический шатёр диаметр 35 м (≈962 м²)",
    description:
      pageData.seo?.description ||
      "Сферический шатёр диаметром 35 метров – грандиозная купольная конструкция для концертов, выставок, спорта, авиаангаров. Просторное внутреннее пространство без опор, высота 35 м. Алюминиевый каркас, ПВХ тент 900 г/м², ветроустойчивость до 120 км/ч.",
    sku: `MOB-${PAGE_CODE}`,
    mpn: `MOB-SPHERE-35M`,
    ...
    additionalProperty: [
      ...
      { "@type": "PropertyValue", name: "Назначение", value: "Концерты, выставки, спорт, авиаангары, склады, производство" },
      { "@type": "PropertyValue", name: "Фундамент", value: "Не требуется (установка на грунт, асфальт, бетон, блоки)" },
      { "@type": "PropertyValue", name: "Срок изготовления", value: "от 20 до 35 рабочих дней" },
      { "@type": "PropertyValue", name: "Гарантия", value: "5 лет на каркас и тент, 25 лет на антикоррозийную обработку стали" },
    ],
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: "Комплектации сферического шатра 35 м",
      itemListElement:
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: pageData.page?.name || "Сферический шатёр диаметр 35 м",
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "Какую площадь имеет сферический шатёр диаметром 35 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Площадь пола составляет примерно 962 м², внутренняя высота – 35 метров. Это позволяет комфортно разместить до 800-900 гостей за столами или до 1200-1400 человек на фуршет, а также проводить концерты, спортивные соревнования и выставки.",
        },
      },
      {
        "@type": "Question",
        name: "Сколько стоит сферический шатёр 35 м под ключ?",
        acceptedAnswer: {
          "@type": "Answer",
          text: `Цена от ${prices[0]?.toLocaleString() || "5 200 000"} ₽ за базовую комплектацию, утеплённая – от ${prices[1]?.toLocaleString() || "7 200 000"} ₽, максимальная (концертный зал/ангар) – от ${prices[2]?.toLocaleString() || "9 500 000"} ₽. Включает каркас, тент, двери, окна. Доставка рассчитывается отдельно.`,
        },
      },
      {
        "@type": "Question",
        name: "Нужен ли фундамент для сферического шатра 35 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Капитальный фундамент не требуется, но из-за большого размера и веса рекомендуется усиленная подготовка площадки (бетонные блоки, утяжелители, возможно мелкозаглубленное основание). Устанавливается на ровную поверхность (асфальт, бетон, утрамбованный грунт).",
        },
      },
      {
        "@type": "Question",
        name: "Можно ли использовать шатёр-сферу 35 м зимой?",
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: "Доставляете ли сферические шатры 35 м в регионы России?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Да, доставляем по всей России через ТК (Деловые линии, ПЭК и др.). Для таких крупногабаритных конструкций организуем спецтранспорт. Возможен выезд на монтаж в любой регион.",
        },
      },
      {
        "@type": "Question",
        name: "Какие мероприятия можно проводить в сфере 35 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Идеален для концертов звёзд, фестивалей, корпоративов, свадеб на 800+ гостей, выставок, ярмарок, спортивных мероприятий (баскетбол, волейбол, теннис), авиационных ангаров, складских комплексов.",
        },
      },
      {
        "@type": "Question",
        name: "Можно ли установить в сфере 35 м окна, двери, сцену?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Да, возможны стеклопакеты ПВХ, входные двери, раздвижные системы, рольставни, а также системы вентиляции, освещения, отопления, сценическое оборудование, акустические панели.",
        },
      },
    ],
  };
  ...
  return (
    <div className="flex flex-col w-full overflow-x-hidden">
      ...
      <h1 className="sr-only">{pageData.seo?.h1 || "Сферический шатёр диаметром 35 метров – космическое пространство для грандиозных задач"}</h1>
