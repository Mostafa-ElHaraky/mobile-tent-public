...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для мероприятий 20×40 м
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для мероприятий 20×40 м",
      text: `Шатёр для мероприятий 20×40 м прекрасно защитит от непогоды и позволит спланировать проведение любого события на природе.
В нашей компании также можно арендовать обогреватели, напольное покрытие, кондиционеры и мебель для шатра.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности, на которую ставится, или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `Конструкция шатра для мероприятий 20×40 м имеет прямоугольную форму и общую площадь 800 м².
Плотный тент пропитан огнеупорной смесью и не выгорает на солнце. Он остаётся в отличном состоянии после большого количества циклов установки и демонтажа.
Каждый клиент может выбирать тип стен — с оконными проёмами, комбинированные или без окон.`,
    },
    {
      title: "Особенности монтажа",
      text: `Конструкция компактная в сложенном состоянии.
Установка производится на ровную поверхность, фундаментная основа не требуется.
Смонтированный шатёр необходимо утяжелить бетонными блоками.
Звоните нам в любое удобное время — с удовольствием предоставим вам шатёр для мероприятий 20×40 м или конструкцию другого размера.`,
    },
  ];
  ...
  const specs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "12800 кг" },
