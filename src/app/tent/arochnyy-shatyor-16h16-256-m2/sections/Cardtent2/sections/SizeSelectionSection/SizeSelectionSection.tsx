...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (16×16 — 256 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Привлекательный и вместительный арочный шатёр размером 16х16 м и площадью 256 м2 можно недорого арендовать для проведения свадьбы или банкета, организации конференций и выставок. При необходимости постоянного использования конструкции рекомендуем купить её по вполне приемлемой цене.`,
    },
    {
      title: "Особенности шатра",
      text: `Модуль оснащен выполненным из оцинкованной стали мощным каркасом, установить который достаточно просто на любой ровной поверхности. Для стенок и крыши шатра использован пожаробезопасный ПВХ-материал, отлично защищающий от промокания и продувания ветром.

Для создания праздничного настроения возможно декоративное оформление тента или размещение на нём рекламных изображений.`,
    },
  ];

  // Default specs from your original code (16×16 — 256 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "3 100 кг" },
    ...
    {
      label: "Модульность",
      value: 'Возможность увеличения площади путём соединения нескольких конструкций со стороной 8 м: "VIP Дюна 8Х8 м","VIP 8Х8 м", "VIP 16Х16 м", "Гексагональ 15Х13 м", "Гексагональ 15Х13 м Long-1", "Гексагональ 15Х13 м LONG-2", "Октогональ 21Х19 м".',
    },
    {
