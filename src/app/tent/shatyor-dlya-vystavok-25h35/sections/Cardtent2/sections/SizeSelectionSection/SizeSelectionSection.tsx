...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для выставок 25×35
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для выставок 25×35 м",
      text: `Шатёр для выставок 25×35 м — практичная и современная конструкция, в которой можно проводить различные культурные мероприятия. Будьте уверены в том, что такое мобильное тентовое здание отлично укроет от солнца или дождя и устоит в сильный ветер.

Компания проектирует, а также изготавливает шатры любых размеров и конфигураций. Специалисты компании проведут качественный ремонт вашего шатра в случае необходимости. Нашу продукцию можно купить или арендовать на нужный вам срок.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности, на которую ставится, или утяжеление конструкции бетонными блоками.`,
    },
    {
      ...
      text: `• Надёжный каркас выполнен из алюминия, а крепёжные элементы — из высококачественной стали.
• Погодные условия и многоразовый монтаж никак не влияют на качество тента.
• Стены шатра могут быть как с окнами, так и без них. Эти нюансы вы можете заблаговременно обсудить перед заказом с нашими специалистами.`,
    },
    {
      title: "Особенности монтажа",
      text: `• Шатёр для выставок размером 25×35 м устанавливается на ровную поверхность. Обустраивать фундамент для этого не требуется.
• После монтажа конструкцию следует закрепить к поверхности и утяжелить бетонными плитами.
• Для заказа свяжитесь с нами по телефону или воспользуйтесь услугой «обратный звонок».`,
    },
  ];

  // TECH SPECS — UPDATED for 25×35
