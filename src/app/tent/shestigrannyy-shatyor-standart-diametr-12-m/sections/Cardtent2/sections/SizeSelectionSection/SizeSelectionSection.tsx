...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шестигранный шатёр Стандарт (Диаметр 12 м)
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title:
        "Шестигранный шатёр Стандарт (Диаметр 12 м) — красивое и удобное решение",
      text: `Шатёр можно оборудовать всем необходимым, чтобы комфортно разместить гостей на природе или улице. Конструкцию закрытого типа можно использовать при любой погоде. Компания — один из крупнейших отечественных производителей каркасно-тентовых конструкций. Шестигранный шатёр «Стандарт» диаметром 12 м можно купить или взять в аренду по цене завода и с гарантией качества.`,
    },
    {
      title: "Преимущества и характеристики",
      text: `• Сбалансированная геометрия и большая площадь — 113 м².
• Долговечная конструкция, которую можно устанавливать и разбирать неограниченное количество раз.
• Транспортировка и монтаж шатра не доставляют сложностей.
• Специальная обработка конструкционных элементов: полотно ПВХ покрыто составом, противостоящим гниению и возгоранию тента; каркас и крепления не ржавеют.
• Возможность увеличения площади путём соединения нескольких конструкций.`,
    },
    {
      title: "Технические параметры (кратко)",
      text: `• Площадь — 113 м²; длина и ширина — по 12 м.
• Высота стен (опоры) — 2,5 / 3 / 3,5 / 4 м; максимальная высота в коньке — 7,5 м.
• Допустимая скорость ветра — до 30 м/сек; нагрузка подвеса на арку — до 30 кг/п.м.
• Вес конструкции — 621 кг.`,
    },
    {
...
• Крепёжные элементы — оцинкованная сталь DIN 50976.`,
    },
    {
      ...
      text: `• Не требует специального фундамента: устанавливается на выровненной земляной поверхности.
• Монтаж и демонтаж выполняются без применения специальной техники, в минимальные сроки и без потери качества; возможны многократные циклы.
• Обязательное крепление конструкции к поверхности или утяжеление бетонными блоками.
• Транспортировка выполняется в сложенном виде; каркас легко складывается, крепежи устойчивы к коррозии.`,
    },
  ];

  // TECH SPECS — UPDATED for Шестигранный шатёр Стандарт (Диаметр 12 м)
  const specs: { label: string; value: string }[] = [
    { label: "Модель", value: "Шестигранный шатёр Стандарт (Диаметр 12 м)" },
    ...
    { label: "Возможность расширения", value: "Соединение нескольких конструкций" },
  ];
