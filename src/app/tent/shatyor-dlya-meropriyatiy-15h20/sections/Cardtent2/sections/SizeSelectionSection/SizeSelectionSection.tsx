...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для мероприятий 15×20 м
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для мероприятий 15×20 м",
      text: `Шатёр для мероприятий 15×20 м –– это компактная и вместительная конструкция.
      
Мы предлагаем:
• продажу;
• аренду;
• ремонт;
• обслуживание шатровых конструкций.`,
    },
    {
...
• Все элементы каркаса выполнены из анодированного алюминия (сплав 6063 DIN 573), сертификат QUALANOD.
...
• Особенности монтажа: обязательное крепление конструкции к поверхности установки или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `• Прямоугольная конструкция с современным дизайном, общая площадь 300 м².
• Тент может быть выполнен в любой цветовой гамме; многоразовый монтаж не влияет на внешний вид.
• Конструкция прочная –– выдерживает порывы ветра, укрывает от дождя и солнца.
• Выбор стен по пожеланиям: с окнами или без, количество оконных проёмов может варьироваться.`,
    },
    {
      title: "Особенности монтажа",
      text: `• Устанавливается на выровненную поверхность.
• В сложенном виде имеет компактные размеры и транспортируется обычной грузовой машиной.
• Специалисты нашей компании доставят и установят шатёр в короткие сроки.
• Свяжитесь с нами по телефону или оставьте сообщение на сайте.`,
    },
  ];
