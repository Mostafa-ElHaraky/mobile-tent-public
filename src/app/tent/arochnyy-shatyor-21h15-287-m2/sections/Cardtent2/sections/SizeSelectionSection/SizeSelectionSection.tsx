...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (21×15 — 287 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Основа арочного шатра 21×15 м — многоугольник, зайти в этот шатёр можно с любой стороны, что облегчает передвижение на мероприятии большого количества людей.`,
    },
    {
      title: "Применение",
      text: `Под большой площадью арочного шатра комфортно будут себя чувствовать все гости мероприятия на открытом воздухе. Несмотря на внушительные размеры, это сборное сооружение легко установить или разобрать, что даёт полную свободу в выборе места проведения праздника.

Если делать сравнение с ценой аренды на 287 м² помещения или шатра, арендовать шатер выгоднее, при этом прокат стоит недорого. Компания MOBILE TENT давно работает в сфере производства навесных конструкций, поэтому может предоставить большой выбор вариантов, в том числе и под заказ.`,
    },
  ];

  // Default specs from your original code (21×15 — 287 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Восьмигранник" },
    { label: "Площадь", value: "287 м²" },
    ...
    {
      label: "Основание",
      value: "Не требует специального фундамента, устанавливаются на любой земляной поверхности. Устанавливается на тумбы высотой 0,5 м.",
    },
    {
      label: "Монтаж",
    ...
    {
      label: "Модульность",
      value: "Возможность увеличения площади путём соединения нескольких конструкций со стороной 8 м.",
    },
    {
