...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Тентовый ангар 20×20
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Тентовый ангар 20×20 — быстрый и выгодный формат помещения",
      text: `Тентовый ангар 20×20 м возводится быстро, готовое помещение подходит для разных видов бизнеса.`,
    },
    {
      title: "Преимущества и характеристики",
      text: `Площадь — 400 м², в коньке максимальная высота — 8,5 м, вес — 6 т.
Подходит для эксплуатации круглый год — особая форма крыши предотвращает скопление атмосферных осадков (воды, снега).
Конструкция обтянута тентовым полотном французского производства (DICKSON); специальные добавки в его составе препятствуют возгоранию и разрушению от воздействия атмосферных факторов.`,
    },
    {
      title: "Сфера применения и преимущества",
      text: `Тентовый ангар 20×20 м может функционировать как полноценное помещение: склад, цех, автомастерская, стоянка для транспорта, спортивная площадка.
Невысокая цена, быстрый монтаж, возможность длительной эксплуатации (до 20 лет).
Прочный каркас из анодированного алюминия и крепежи из оцинкованной стали устойчивы к коррозии.`,
    },
    {
      title: "Особенности монтажа",
      text: `Тентовый ангар 20×20 м возводится без обустройства фундамента и использования спецтехники. Необходим минимум разрешений (ангар не является капитальным сооружением).
Возможен монтаж на любой ровной поверхности.
При необходимости ангар можно быстро демонтировать и перевезти.`,
    },
  ];

  // TECH SPECS — UPDATED for Тентовый ангар 20×20
  const specs: { label: string; value: string }[] = [
    { label: "Модель", value: "Тентовый ангар 20×20" },
    ...
    { label: "Вес конструкции", value: "6000 кг" },
