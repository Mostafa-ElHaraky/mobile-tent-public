...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING CONTENT — UPDATED for Мобильный шатёр Hard Prof 3х3
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Мобильный шатёр Hard Prof 3х3",
      text: `Купить небольшой шатер 3х3 м — означает получить универсальную навесную конструкцию, которая защитит от солнца и дождя. Под таким шатром комфортно отдыхать на даче или проводить уличное мероприятие.`,
    },
    {
      title: "Доступная покупка и аренда",
      text: `Наши цены на аренду и покупку шатра размером 3х3 м вполне доступные из-за его компактных размеров и простоты конструкции. Такой открытый шатёр хорошо установить на природе для небольшой компании гостей в тёплый период года.`,
    },
    {
      title: "Качество от производителя",
      text: `Покупая тентовые конструкции от производителя, Вы всегда будете уверены в их качестве. Компания MOBILE TENT поможет подобрать комплексное решение для создания яркого праздника на открытом воздухе, торговли или ресторанного бизнеса.`,
    },
    {
      title: "Монтаж",
      text: `Наши конструкции просты в монтаже, но всё же некоторый опыт для выполнения таких работ желателен. При заказе аренды или покупке шатра для качественного монтажа рекомендуем пригласить специалиста.`,
    },
  ];

  // TECH SPECS — UPDATED for Hard Prof 3х3
  const specs: { label: string; value: string }[] = [
    { label: "Серия/Модель", value: "Hard Prof 3х3" },
    ...
    { label: "Размер упаковки", value: "163 x 32 x 32 см" },
