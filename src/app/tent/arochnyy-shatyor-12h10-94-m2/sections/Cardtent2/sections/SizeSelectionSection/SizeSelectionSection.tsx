...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (12×10 — 94 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Плавность линий купола арочного шатра заставляет обратить на себя внимание. Такая конструкция не останется незамеченной, будет центральным элементом любого ивента на открытом воздухе.

Гармоничные пропорции арочного шатра позволяют получить достаточно большую площадь, укрытую под куполом (94 м²). В основе конструкции находится многоугольник, благодаря такому решению удалось достичь максимального пространства.`,
    },
    {
      title: "Применение",
      text: `Арочный шатёр размером 12×10 м и площадью 94 м² станет прекрасной площадкой для организации праздника на открытом воздухе. Арендовать его выгоднее, чем снять аналогичное по площади помещение.`,
    },
  ];

  // Default specs from your original code (12×10 — 94 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Шестигранник" },
    { label: "Площадь", value: "94 м²" },
