...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your code (10×35 — 350 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `У нас можно купить или взять в аренду классический шатёр размером 10×35 м. Компания предлагает полный спектр услуг по обслуживанию шатров и проводит ремонтные работы.`,
    },
    {
      title: "Характеристики и преимущества",
      text: `Классический шатёр 10×35 м имеет прямоугольную форму и современный дизайн, который отлично выглядит в любом месте:
• длина шатра — 35 м,
• ширина — 10 м,
• общая площадь — 350 м².

Благодаря наличию окон и дверей шатёр отлично подойдёт для обустройства летнего кафе. В нём возможно проведение культурных мероприятий, праздников и торжеств на природе.

Тентовое полотно и стены изготовлены из ПВХ французского производства.`,
    },
    {
      title: "Особенности монтажа",
      text: `• Каркасная конструкция собирается и монтируется достаточно просто.
• Установка выполняется на любую ровную поверхность, без обустройства фундамента.
• Для надёжности требуется закрепить конструкцию и/или утяжелить её бетонными блоками.

В компании MOBILE TENT вы можете оформить заказ на покупку или аренду каркасно-тентовой конструкции.`,
    },
  ];

  // Default specs from your code (10×35 — 350 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "1 925 кг" },
