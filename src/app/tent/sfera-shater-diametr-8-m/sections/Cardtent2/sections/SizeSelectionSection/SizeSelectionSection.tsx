...
export const SizeSelectionSection = ({ 
  ...
  // Default content for Сфера шатер диаметр 8 м (≈50 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Компания MOBILE TENT",
      text: "Компания MOBILE TENT специализируется на производстве быстровозводимых сферических шатров и купольных конструкций. Сфера шатер диаметром 8 метров (≈50 м²) — это уникальное купольное сооружение, которое привлекает внимание своей эстетикой и функциональностью. Идеально для мероприятий, выставочных павильонов, кафе, ресторанов, складов и временных объектов.",
    },
    {
      title: "Особенности модели",
      text: "Сферический шатер выполнен в форме полусферы, что обеспечивает максимальную прочность конструкции и равномерное распределение нагрузок. Диаметр 8 метров даёт полезную площадь около 50 м² и высоту в центре до 8 метров. Конструкция не требует капитального фундамента, монтируется за 2–3 дня. Благодаря утеплённой версии шатер можно использовать круглый год.",
    },
    {
      title: "Практичность и применение",
      text: "Сфера‑шатер 8 м идеально подходит для: свадебных банкетов, дней рождения, корпоративов, фуршетов, выставок, ярмарок, сезонных кафе, ресторанов под открытым небом, торговых точек, временных магазинов, складских помещений, укрытия техники, фотостудий, выставочных залов, лекционных залов, коворкингов, лаунж‑зон на фестивалях.",
    },
    {
      title: "Материалы",
    ...
    },
    {
      ...
      text: "Возможность соединения нескольких сфер в единый комплекс (кластер). Можно комбинировать с шатрами других форм и размеров. Сферические шатры легко масштабируются: диаметры 6 м, 8 м, 10 м, 12 м и более.",
    },
  ];

  // Technical specifications for sphere tent diameter 8 m
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Сферическая (полусфера)" },
    { label: "Диаметр", value: "8 м" },
    { label: "Площадь пола", value: "≈50 м²" },
    { label: "Высота в центре", value: "8 м" },
    { label: "Высота стенки", value: "0 м (купол)" },
    { label: "Вес металлоконструкции (алюминий)", value: "~450 кг" },
    { label: "Вес металлоконструкции (сталь)", value: "~1200 кг" },
    ...
    { label: "Монтаж", value: "2–3 дня профессиональной бригадой" },
