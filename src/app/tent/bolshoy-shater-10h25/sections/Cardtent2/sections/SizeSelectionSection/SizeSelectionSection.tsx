...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your code (10×25 — 250 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title:
        "Надёжная мобильная конструкция 10×25 м — 250 м²",
      text: `Надёжная мобильная конструкция, которую легко перевезти на любое место и установить — это классический шатёр 10×25 м. Он заменит полноценное помещение и обеспечит комфорт на открытом воздухе.

В нашем каталоге представлен широкий выбор навесов, шатров и тентов, а также дополнительное оборудование для них. Производственные мощности предприятия позволяют изготавливать стандартные конфигурации, а также конструкции под заказ по индивидуальным эскизам.`,
    },
    {
      title: "Технические характеристики и особенности конструкции",
      text: `• Классический шатёр 10×25 м не требует заливки фундамента и специального разрешения на установку.
• Вес конструкции составляет 1 375 кг, каркас рассчитан на нагрузку до 30 кг/п.м. и выдержит порывы ветра до 30 м/сек.
• Полотно французского производства пропитано составом, предотвращающим гниение. Оно устойчиво к воздействию воды и пожаробезопасно.`,
    },
    {
      title: "Монтаж",
      text: `• Классический шатёр 10×25 м необходимо закреплять на поверхности, утяжеляя бетонными блоками.
• Рассчитан на многоразовый монтаж/демонтаж.
• Все элементы конструкции надёжно стыкуются между собой, для установки не нужно задействовать спецтехнику.`,
    },
    {
      title: "Гарантии и сервис",
      text: `Мы работаем со 100% гарантией качества и соблюдаем сервисные обязательства в полном объёме.`,
    },
    {
  ...
  // Default specs from your code (10×25 — 250 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "1 375 кг" },
    ...
    },
    {
      ...
      value:
        "Тентовое полотно и стены из ПВХ французского производства DICKSON с системой LoWick. Класс пожарной безопасности М2, Г1. Тентовое покрытие — сертифицированное ПВХ-полотно с добавками, препятствующими горению (DIN 4102 B1, M2), 0,3 кН/м², устойчиво к гниению.",
    },
    {
      label: "Каркас",
      value:
    ...
    {
      label: "Крепёж",
      value: "Оцинкованная сталь DIN 50976.",
    },
    {
      label: "Особенности монтажа",
      value:
        "Обязательное крепление к поверхности или утяжеление бетонными блоками.",
    },
  ];
