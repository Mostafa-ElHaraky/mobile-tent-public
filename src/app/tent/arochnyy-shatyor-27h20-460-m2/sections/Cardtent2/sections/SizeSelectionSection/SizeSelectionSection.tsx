...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your code (27×20 — 460 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Арочный шатёр 27×20 м — 460 м² представляет собой внушительную модульную конструкцию в форме арки. 
Компания MOBILE TENT рекомендует купить такое изделие для проведения выставок, корпоративных мероприятий, свадебных торжеств, организации кафе на открытом воздухе. 
Для использования шатра один или несколько раз у Вас имеется возможность недорого арендовать его, что позволит защитить любое мероприятие от непогоды за доступную цену.`,
    },
    {
      title: "Особенности модели",
      text: `Особенностью арочного шатра 460 м² является стильный дизайн, легкость транспортировки и установки.`,
    },
    {
      title: "Особенности монтажа",
      text: `Модуль не нуждается в фундаменте и может размещаться в любом месте. 
Изготовленный из оцинкованной стали каркас надёжно удерживает прозрачное тентовое полотно из ПВХ, которое при желании можно заменить на белое.`,
    },
  ];

  // Default specs from your code (27×20 — 460 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Восьмигранник" },
    { label: "Площадь", value: "460 м²" },
    { label: "Длина", value: "27 м" },
    { label: "Ширина", value: "20 м" },
    { label: "Высота входной арки", value: "4,5 м" },
    { label: "Высота в коньке", value: "6,3 м" },
    { label: "Вес конструкции", value: "4 500 кг" },
    { label: "Нагрузка на опорную пятку", value: "562 кг" },
    ...
    },
    {
      label: "Модульность",
      value:
        "Возможность увеличения площади путём соединения нескольких конструкций со стороной 5 м (20×22 м, 20×27 м, 20×32 м и т. д.).",
    },
    {
