...
const PAGE_CODE = "sfera-shater-diametr-14-m";
...
async function getPageData(): Promise<PageData> {
  try {
    ...
    // Fallback data for spherical tent diameter 14 m
    return {
      success: false,
      page: {
        id: 0,
        name: "Сфера шатер диаметр 14 м (≈154 м²)",
        code: PAGE_CODE,
      },
      seo: {
        title: "Сфера шатер диаметр 14 м (154 м²) – купить сферический шатер в Москве | MOBILE TENT",
        description:
          "Сферический шатер диаметром 14 метров (площадь ≈154 м²) от производителя MOBILE TENT. Уникальная купольная конструкция для крупных мероприятий, ресторанов, выставок, складов. Алюминиевый каркас, ПВХ тент 900 г/м². Цена от 1 050 000 ₽. Гарантия 5 лет. Доставка по Москве, МО и всей России.",
        keywords:
          "сфера шатер диаметр 14 м, сферический шатер 14 метров, купить сферический шатер, шатер купол 14 м, геодезический купол 14 м, шатер сфера 154 м², тентовый купол 14 м, шатер для мероприятий большой, шатер ресторан, шатер выставка",
        h1: "Сфера шатер диаметром 14 метров – купольное пространство для крупных проектов",
        url: `/tent/${PAGE_CODE}`,
      },
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = data.seo?.title || "Сфера шатер диаметр 14 м (154 м²) – купить сферический шатер в Москве | MOBILE TENT";
  const description =
    data.seo?.description ||
    "Сферический шатер диаметром 14 метров (площадь ≈154 м²) от производителя MOBILE TENT. Уникальная купольная конструкция для крупных мероприятий, ресторанов, выставок, складов. Алюминиевый каркас, ПВХ тент 900 г/м². Цена от 1 050 000 ₽. Гарантия 5 лет. Доставка по Москве, МО и всей России.";
  const keywords =
    data.seo?.keywords ||
    "сфера шатер диаметр 14 м, сферический шатер 14 метров, купить сферический шатер, шатер купол 14 м, геодезический купол 14 м";
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: "Сфера шатер диаметр 14 метров",
        },
      ],
    },
    robots: {
...
export default async function SphereTentPage() {
  ...
  const prices = pageData.priceConfigurations?.map((c) => c.currentPrice) || [1050000, 1450000, 1890000];
  const lowPrice = Math.min(...prices);
  const highPrice = Math.max(...prices);
  const offerCount = pageData.priceConfigurations?.length || 3;
  const diameter = 14;
  const area = 154; // π × (7²) = 153.94 ≈ 154 m²
  const height = 14; // full sphere height equals diameter

  const productSchema = {
    "@context": "https://schema.org",
    "@type": "Product",
    image: "https://mobile-tent.ru/spher26,1.webp",
    name: pageData.page?.name || "Сфера шатер диаметр 14 м (≈154 м²)",
    description:
      pageData.seo?.description ||
      "Сферический шатер диаметром 14 метров – купольная конструкция для крупных мероприятий, ресторанов, складов, выставок. Просторное внутреннее пространство без опор, высота 14 м. Алюминиевый каркас, ПВХ тент 900 г/м², ветроустойчивость до 120 км/ч.",
    sku: `MOB-${PAGE_CODE}`,
    mpn: `MOB-SPHERE-14M`,
    ...
    additionalProperty: [
      ...
      { "@type": "PropertyValue", name: "Назначение", value: "Крупные мероприятия, свадьбы, рестораны, выставки, склады, укрытие техники" },
    ...
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: "Комплектации сферического шатра 14 м",
      itemListElement:
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: pageData.page?.name || "Сфера шатер диаметр 14 м",
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "Какую площадь имеет сферический шатер диаметром 14 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Площадь пола составляет примерно 154 м², внутренняя высота – 14 метров. Это позволяет комфортно разместить до 120-140 гостей за столами или до 180-200 человек на фуршет.",
        },
      },
      {
        "@type": "Question",
        name: "Сколько стоит сфера шатер 14 м под ключ?",
        acceptedAnswer: {
          "@type": "Answer",
          text: `Цена от ${prices[0]?.toLocaleString() || "1 050 000"} ₽ за базовую комплектацию, утеплённая – от ${prices[1]?.toLocaleString() || "1 450 000"} ₽, максимальная (ресторан/офис) – от ${prices[2]?.toLocaleString() || "1 890 000"} ₽. Включает каркас, тент, двери, окна. Доставка рассчитывается отдельно.`,
        },
      },
      {
        "@type": "Question",
        name: "Нужен ли фундамент для сферического шатра 14 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Нет, капитальный фундамент не требуется. Устанавливается на ровную площадку (грунт, асфальт, бетон, бетонные блоки) с фиксацией анкерами. Для увеличенной устойчивости могут использоваться бетонные утяжелители.",
        },
      },
      {
        "@type": "Question",
        name: "Можно ли использовать шатер-сферу 14 м зимой?",
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: "Доставляете ли сферические шатры 14 м в регионы?",
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: "Какие мероприятия можно проводить в сфере 14 м?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Идеален для крупных свадеб, банкетов на 100+ гостей, корпоративов, выставок, концертов, ярмарок, ресторанов, временных магазинов, складов, укрытия техники.",
        },
      },
      {
        "@type": "Question",
        name: "Можно ли установить в сфере 14 м окна и двери?",
        acceptedAnswer: {
          "@type": "Answer",
  ...
  return (
    <div className="flex flex-col w-full overflow-x-hidden">
      ...
      <h1 className="sr-only">{pageData.seo?.h1 || "Сфера шатер диаметром 14 метров – купольное пространство для крупных проектов"}</h1>
