...
export const SizeSelectionSection = ({ 
  ...
  // Default content for Сфера шатер диаметр 14 м (≈154 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Компания MOBILE TENT",
      text: "Компания MOBILE TENT специализируется на производстве быстровозводимых сферических шатров и купольных конструкций. Сфера шатер диаметром 14 метров (≈154 м²) — это впечатляющее купольное сооружение для крупных мероприятий, ресторанов, выставочных павильонов, складов и временных объектов. Уникальная геодезическая форма обеспечивает максимальную прочность при минимальном весе.",
    },
    {
      title: "Особенности модели",
      text: "Сферический шатер выполнен в форме полусферы, что даёт равномерное распределение нагрузок. Диаметр 14 метров создаёт полезную площадь около 154 м² и высоту в центре до 14 метров. Конструкция не требует капитального фундамента, монтируется за 5–7 дней. Благодаря утеплённой версии шатер можно использовать круглый год даже при -40°C.",
    },
    {
      title: "Практичность и применение",
      text: "Сфера‑шатер 14 м идеально подходит для: крупных свадеб и банкетов (до 140 гостей за столами), корпоративов, фуршетов (до 200 человек), выставок, концертов, ярмарок, ресторанов под открытым небом, торговых точек, складов, укрытия техники, фотостудий, коворкингов, лаунж‑зон на фестивалях, спортивных мероприятий.",
    },
    {
      title: "Материалы",
    ...
    },
    {
      ...
      text: "Возможность соединения нескольких сфер в единый комплекс (кластер). Можно комбинировать с шатрами других форм и размеров. Сферические шатры легко масштабируются: диаметры 6 м, 8 м, 10 м, 12 м, 14 м, 16 м и более.",
    },
  ];

  // Technical specifications for sphere tent diameter 14 m
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Сферическая (полусфера)" },
    { label: "Диаметр", value: "14 м" },
    { label: "Площадь пола", value: "≈154 м²" },
    { label: "Высота в центре", value: "14 м" },
    { label: "Высота стенки", value: "0 м (купол)" },
    { label: "Вес металлоконструкции (алюминий)", value: "~1 200 кг" },
    { label: "Вес металлоконструкции (сталь)", value: "~2 800 кг" },
