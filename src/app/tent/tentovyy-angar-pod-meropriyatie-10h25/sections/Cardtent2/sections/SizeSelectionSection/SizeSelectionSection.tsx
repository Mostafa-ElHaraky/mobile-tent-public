...
// MARKETING / DESCRIPTION — UPDATED for Тентовый ангар под мероприятие 10×25
const descriptionBlocks: { title?: string; text: string }[] = [
  {
    title: "Тентовый ангар под мероприятие 10×25 — 250 м² функционального пространства",
    text: `Каркасно-тентовые ангары всё больше пользуются популярностью — не требуется разрешение на капитальное строительство, а стоимость в разы ниже стационарных зданий.

Тентовый ангар 10×25 м от компании — надёжный, выгодный и удобный. Мы выполняем все работы: от проектирования до сдачи объекта «под ключ».`,
  },
  {
    title: "Преимущества и характеристики",
    text: `Прямоугольный тентовый ангар 10×25 м отлично выдерживает ветровые и снеговые нагрузки. Подходит как торговое или складское помещение, хранилище для овощей и зерна, а также для организации животноводческого комплекса.

ПВХ-покрытие прочное, не выгорает на солнце, не трескается, устойчиво к гниению и горению.
Предлагаем дополнительное оборудование: полы, освещение, кондиционирование и вентиляцию, мебель.`,
  },
  {
    title: "Особенности монтажа",
    text: `Монтаж производится быстро и не требует специальной техники. Ангар устанавливается на ровную земляную или асфальтовую поверхность без фундамента. После монтажа конструкцию рекомендуется утяжелить и закрепить к поверхности.

Продукцию компании можно купить или арендовать на нужный вам срок.`,
  },
];

// TECH SPECS — UPDATED for Тентовый ангар 10×25
const specs: { label: string; value: string }[] = [
  { label: "Модель", value: "Тентовый ангар 10×25 (под мероприятие)" },
  ...
  { label: "Профиль прогонов", value: "50 × 50 × 3,00 мм, сертификат QUALANOD" },
  {
    ...
    value: "Не требует специального фундамента; установка на любой земляной или асфальтовой поверхности.",
  },
  {
  ...
  },
  {
    ...
    value: "Возможность увеличения площади путём соединения нескольких конструкций.",
  },
  {
