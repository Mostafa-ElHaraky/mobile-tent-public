...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (10×3 — 30 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Применение",
      text: `Арочный шатёр размером 10×3 м представляет собой крытую конструкцию, которая подойдет для проведения мероприятий на открытом воздухе. 
Шатёр может использоваться в качестве защищающего от непогоды навеса для сцены или места отдыха на открытой площадке. 
Имеется возможность арендовать арочный шатёр для разового мероприятия.

Используется как дополнительный модуль под сцену, танц. пол, зона молодожен, детская игровая зона, пультовая для DJ.`,
    },
    {
      title: "Монтаж и конструкция",
      text: `Монтаж конструкции производится достаточно просто и быстро на любой ровной поверхности. 
Выполненный из алюминия и оцинкованной стали каркас отлично выдерживает нагрузку тентового полотна из ПВХ.`,
    },
    {
      title: "Возможности объединения",
      text: `Есть возможность объединения нескольких конструкций в одну для увеличения площади.`,
    },
  ];

  // Default specs from your original code (10×3 — 30 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Прямоугольник" },
    { label: "Площадь", value: "30 м²" },
    { label: "Длина", value: "10 м" },
    { label: "Ширина", value: "3 м" },
    { label: "Высота входной арки", value: "4,5 м" },
    { label: "Высота в коньке", value: "4,5 м" },
    { label: "Вес конструкции", value: "690 кг" },
    { label: "Нагрузка на опорную пятку", value: "172 кг" },
    ...
    {
      label: "Модульность",
      value: 'Возможность увеличения площади путём соединения нескольких конструкций со стороной 10 м: "VIP 10×10 м", "VIP Дюна 10×10 м", "VIP Гексагональ 20×17 м", "VIP Гексагональ 20×17 м Long-1", "VIP Гексагональ 20×17 м Long-2", "VIP 20×20 м", "VIP 20×25 м", "VIP Октогональ 26×24 м", "VIP Дополнительный модуль 10×3 м", "VIP Дополнительный модуль 10×5 м".',
    },
    {
