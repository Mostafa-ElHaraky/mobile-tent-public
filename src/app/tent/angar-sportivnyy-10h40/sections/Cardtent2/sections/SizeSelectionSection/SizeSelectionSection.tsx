...
export const SizeSelectionSection = ({ 
  ...
  // Default content for Ангар спортивный 10×40 (400 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Ангар спортивный 10×40",
      text: "Быстровозводимые сооружения идеальны для малого и среднего бизнеса. На этапе развития важно получить помещение с невысокой стоимостью, которое будет находиться в собственности предприятия. Тентовый ангар 10×40 м доступен по цене, а материалы высокого качества обеспечивают длительный срок службы.",
    },
    {
      title: "Преимущества и применение",
      text: "Тентовый ангар 10×40 м может стать торговым павильоном, спортивным комплексом, мастерской, цехом или складским помещением. Конструкция подходит для круглогодичного использования и обеспечивает минимальные затраты на монтаж и обустройство.",
    },
    {
      title: "Практичность и монтаж",
      text: "Не требует специального фундамента — установка возможна на любую ровную земляную или асфальтовую поверхность. Монтируется и демонтируется без применения спецтехники в минимальные сроки, многократно, без потери качества. Возможна модульная стыковка нескольких конструкций для увеличения площади. Особенности монтажа: обязательное крепление к основанию или утяжеление бетонными блоками. Поставка осуществляется в сложенном виде, готовом к сборке.",
    },
    {
      title: "Материалы тента",
      text: "Тентовое полотно и стены изготовлены из ПВХ материала французского производства DICKSON с системой LoWick. Класс пожарной безопасности — М2, Г1. Тентовое покрытие — сертифицированное ПВХ-полотно с добавками, препятствующими горению (DIN 4102 B1, M2), прочность 0,3 кН/м², устойчиво к гниению.",
    },
    {
    ...
    {
      title: "Опции и расширение",
      text: "Площадь можно расширять соединением с другими навесными конструкциями. Возможно дополнительное оснащение системами отопления и кондиционирования.",
    },
  ];
  ...
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Длина", value: "40 м (регулируется)" },
    ...
    { label: "Вес конструкции", value: "2 200 кг (алюминий)" },
