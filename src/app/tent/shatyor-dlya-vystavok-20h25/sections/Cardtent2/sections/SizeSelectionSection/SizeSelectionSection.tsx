...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для выставок 20×25
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для выставок 20×25 м",
      text: `Шатёр для выставок 20×25 м имеет современный дизайн и привлекательный внешний вид. Конструкция закрытого типа способна поместить много людей, и ваше мероприятие пройдёт на высоком уровне.

Российское предприятие проектирует и изготавливает, а также реализует и сдаёт в аренду каркасно-тентовую продукцию для организации выставок любых конфигураций и размеров.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности, на которую ставится, или утяжеление конструкции бетонными блоками.`,
    },
    {
      ...
      text: `• Надёжный каркас и прочное ПВХ способны устоять при непогоде.
• Респектабельный внешний вид — шатёр выглядит презентабельно, доступна аренда напольного покрытия, обогревателей и других опций.
• Конструкция 20×25 м легко выдерживает многократный монтаж/демонтаж без потери первоначальных качеств.`,
    },
    {
      title: "Особенности монтажа",
      text: `Заказывая у нас шатёр для выставок 20×25 м, вы также получаете доставку и установку конструкции на месте проведения мероприятия.

• Установка шатра не требует спецтехники и закладки фундамента.
• В сложенном виде конструкция имеет компактные габариты и доставляется обычным грузовым транспортом.
• После монтажа каркас рекомендуется утяжелить бетонными плитами.`,
    },
  ];
