...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для мероприятий 25×25 м
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для мероприятий 25×25 м",
      text: `Квадратная компактная конструкция шатра для мероприятий 25×25 м способна вместить внутри большое количество людей.
Наши специалисты спроектируют и изготовят каркасно-тентовую конструкцию под ваши задачи. Компания также предоставляет услуги по ремонту и обслуживанию шатров, а также аренду оборудования (обогреватели, напольное покрытие, кондиционеры, мебель). Подробную информацию вы можете получить у наших менеджеров.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности, на которую ставится, или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `Шатёр для мероприятий 25×25 м подойдет для организации торжеств, выставок, спортивных соревнований и праздников.
Конструкция отлично выдерживает порывы ветра, укроет в дождь и защитит от палящего солнца.
Каркас выполнен из прочного материала. Цвет тента может быть любым. По желанию нанесём логотип вашей компании на поверхность тента.`,
    },
    {
      title: "Особенности монтажа",
      text: `Фундамент не требуется — шатёр устанавливается на ровную поверхность.
Специалисты нашей компании доставят и смонтируют шатёр 25×25 м в кратчайшие сроки.
