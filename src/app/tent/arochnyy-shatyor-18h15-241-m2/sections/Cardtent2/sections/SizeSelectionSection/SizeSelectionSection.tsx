...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (18×15 — 241 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Представленный оригинальный арочный шатёр размером 18×15 м и площадью 241 м² незаменим при проведении больших мероприятий, а также подходит для организации торговых павильонов, размещения летних баров и кафе.`,
    },
    {
      title: "Особенности модели",
      text: `Прочный оцинкованный каркас модели легко собирается и размещается в любом подходящем по площади месте. Шатёр может быть укомплектован прозрачными или глухими стенами из ПВХ.

У Вас имеется возможность недорого арендовать шатёр, что выгодно для проведения свадебных торжеств и корпоративных мероприятий. Для постоянного ведения торговой и выставочной деятельности рекомендуем купить конструкцию. Этот качественный, привлекательный и легко монтируемый шатёр обязательно принесёт прибыль своему владельцу!`,
    },
  ];

  // Default specs from your original code (18×15 — 241 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Восьмигранник" },
    { label: "Площадь", value: "241 м²" },
    { label: "Длина", value: "18,7 м" },
    ...
    { label: "Нагрузка на опорную пятку", value: "550 кг" },
    ...
    {
      label: "Модульность",
      value: "Возможность увеличения площади путём соединения нескольких конструкций со стороной 10 м.",
    },
    {
