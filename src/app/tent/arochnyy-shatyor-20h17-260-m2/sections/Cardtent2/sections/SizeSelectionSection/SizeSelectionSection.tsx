...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (20×17 — 260 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Организация торжеств с большим количеством людей требует помещений существенных размеров. Компания рекомендует купить или взять недорого в аренду одну из самых больших тентовых конструкций — арочный шатер 20×17 м, позволяющий провести мероприятие на открытом воздухе.

Приобретение шатра по оптимальной цене даст возможность избежать зависимости от непогоды и разместить приглашённых в любом подходящем месте. Оригинальный шатёр имеет форму шестигранника.`,
    },
    {
      title: "Особенности монтажа",
      text: `Выполненный из оцинкованного металла каркас легко устанавливается. Тентовое покрытие изготовлено из прочного, пожаробезопасного ПВХ белого цвета. Для размещения шатра необходима ровная земляная, асфальтированная или деревянная поверхность.`,
    },
  ];

  // Default specs from your original code (20×17 — 260 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    { label: "Форма", value: "Шестигранник" },
    { label: "Площадь", value: "260 м²" },
    { label: "Длина", value: "20 м" },
    { label: "Ширина", value: "17 м" },
