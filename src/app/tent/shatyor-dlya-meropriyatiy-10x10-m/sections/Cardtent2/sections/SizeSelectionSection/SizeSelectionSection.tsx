...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для мероприятий 10×10 м
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для мероприятий 10×10 м",
      text: `Тент 10×10 м — компактная квадратная конструкция площадью 100 м², которая эффективно защищает гостей от дождя, снега и солнечных лучей и подходит для широкого спектра событий.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности установки или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `• свадьбы;
• корпоративы;
• семейные вечеринки и юбилеи;
• ярмарки и выставки;
• спортивные мероприятия и многое другое.`,
    },
    {
      title: "Аренда — удобно и выгодно",
      text: `Если шатёр нужен для одного небольшого мероприятия (≈50 человек) и вы не хотите тратить лишние средства, предлагаем аренду. Вам не придётся думать о хранении и дальнейшей эксплуатации — обо всём позаботятся наши специалисты.`,
    },
    {
      title: "Качество и дополнительные услуги",
      text: `Большую часть шатров мы изготавливаем самостоятельно, что гарантирует высокое качество и надёжность. Дополнительно предлагаем:
• обслуживание и ремонт шатров;
• установку пола, освещения, кондиционирования или обогрева;
• декорирование;
• установку мебели и многое другое.`,
    },
    {
      title: "Доставка и поддержка",
      text: `Если вы закажете шатёр в компании Mobile Tent, наши специалисты привезут его в любой город РФ, установят и дадут подробную консультацию по эксплуатации. Остались вопросы? Позвоните нашим менеджерам по номеру, указанному на сайте — мы обязательно поможем!`,
    },
  ];

  // TECH SPECS — UPDATED for 10×10
