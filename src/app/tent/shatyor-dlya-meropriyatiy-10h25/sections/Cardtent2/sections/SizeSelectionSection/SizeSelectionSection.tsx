...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для мероприятий 10×25 м
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для мероприятий 10×25 м",
      text: `Шатёр 10×25 м –– высококачественный мобильный тент. Он позволит сэкономить на аренде помещения и разместиться на открытом воздухе в удобном для вас месте. Тент надёжно защищает от ветра и дождя, а также препятствует попаданию внутрь прямых солнечных лучей. Полезная площадь — 250 м².`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности установки или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `Шатёр для мероприятий 10×25 м имеет площадь 250 м². Тент решает важную задачу оформления пространства на мероприятиях разного формата. Этот шатёр может применяться для проведения любых событий: спортивных, корпоративных, презентационных, музыкальных, культурных.

Для изготовления используется ПВХ французского производителя DICKSON с системой LoWick. Материал непромокаемый, не подвержен горению и гниению.

Каркас изготовлен из сплава алюминия и имеет сертификацию QUALANOD. Элементы крепежа произведены из оцинкованной стали DIN 50976.`,
    },
    {
      title: "Услуги",
      text: `• Проектирование и изготовление шатров и навесов.
• Продажа и аренда шатров.
• Обслуживание и ремонт шатровых конструкций.`,
    },
    {
      title: "Особенности монтажа",
      text: `При установке тентов необходимо учесть особенности сборки и демонтажа мобильных шатров:
• Быстровозводимые конструкции не требуют фундамента и устанавливаются на земляной поверхности.
• Монтаж выполняется в минимальные сроки и без применения специализированной техники.
• Необходимо крепление конструкции к поверхности или утяжеление при помощи бетонных плит.`,
    },
  ];

  // TECH SPECS — UPDATED for 10×25
  const specs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "1375 кг" },
