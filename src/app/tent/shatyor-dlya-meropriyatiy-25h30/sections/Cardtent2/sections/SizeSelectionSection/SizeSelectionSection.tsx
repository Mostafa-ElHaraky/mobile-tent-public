...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для мероприятий 25×30 м
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для мероприятий 25×30 м",
      text: `Любое запланированное мероприятие вполне может испортить плохая погода. Чтобы этого не случилось, правильным решением будет купить или взять в аренду шатёр для мероприятий большого размера, например, 25×30 м. Производством и проектированием каркасно-тентовых конструкций занимается российская компания MOBILE TENT.
У нас можно заказать дополнительное оборудование, напольное покрытие и мебель для шатров.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности, на которую ставится, или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `Большой и вместительный шатёр для мероприятий 25×30 м имеет современный дизайн и красивый внешний вид.
Производить монтаж шатра можно большое количество раз, от этого тент не теряет прочностных характеристик.
Вы можете выбрать тип стен — с окнами или без.
Цвет шатра может быть любым. Возможно брендирование поверхности тента.`,
    },
    {
      title: "Особенности монтажа",
      text: `Заказанный у нас шатёр для мероприятий 25×30 мы поможем доставить на место проведения события и установить.
Собранный шатёр следует надёжно закрепить к поверхности и утяжелить.
Оформить заказ можно на сайте или позвонить нам по телефону.`,
    },
  ];
