...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шатёр для мероприятий 15×35 м
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатёр для мероприятий 15×35 м",
      text: `Наше предприятие готово выполнить ваш заказ на каркасно-тентовую продукцию любых размеров и конфигураций.

Шатёр для мероприятий 15×35 м — компактная переносная конструкция общей площадью 525 м², способная укрыть от дождя, ветра и солнца большое количество гостей. Сделав выбор в пользу шатра, вы будете уверены в отличном проведении праздника или другого события.

Нашу продукцию вы можете купить или арендовать.`,
    },
    {
      title: "Преимущества и характеристики",
      text: `• Вместительный шатёр для мероприятий 15×35 имеет прямоугольную форму и общую площадь 525 м².
• Прочный тент не теряет своих качеств после многоразового монтажа, не выгорает под палящим солнцем; материал огнеупорный.
• Количество окон и дверей может изменяться по желанию клиента.
• Шатёр можно изготовить в любом цвете, возможно брендирование поверхности тента.`,
    },
    {
      title: "Особенности монтажа",
      text: `• Для установки шатра не требуется фундамент — подойдёт любая ровная поверхность.
• После монтажа конструкцию рекомендуется утяжелить.
• Свяжитесь с нами — мы изготовим, доставим и произведём монтаж шатра для мероприятий 15×35 быстро и качественно.`,
    },
    {
...
• Особенности монтажа: обязательное крепление конструкции к поверхности или утяжеление бетонными блоками.`,
    },
    {
      ...
      text: `Закажите шатёр для мероприятий 15×35 у нас — мы быстро рассчитаем смету, организуем доставку и монтаж.`,
    },
  ];

  // TECH SPECS — UPDATED for 15×35
  const specs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "2888 кг" },
