...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your original code (12×12 — 144 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Практичный и универсальный арочный шатёр размером 12×12 м представляет собой модуль квадратной формы, к которому можно пристроить аналогичные конструкции. Компания предлагает купить шатёр по оптимальной цене для проведения массовых мероприятий.

Имеется возможность недорого арендовать модуль, что позволит создать максимально комфортные условия для празднования свадьбы, организации конференции или банкета. Арочный шатёр изготовлен из качественных материалов, что обеспечивает ему прочность и отличную устойчивость.`,
    },
    {
      title: "Особенности монтажа",
      text: `Конструкция без особых усилий собирается и демонтируется, легко транспортируется. Шатёр не требует дополнительных креплений к поверхности, может быть установлен на газоне, асфальте или земле.`,
    },
  ];

  // Default specs from your original code (12×12 — 144 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Высота в коньке", value: "4,3 м" },
    { label: "Вес конструкции", value: "2000 кг" },
    { label: "Нагрузка на опорную пятку", value: "250 кг" },
    ...
    {
      label: "Модульность",
      value: "Возможность увеличения площади путём соединения нескольких конструкций со стороной 6 м: «VIP 6×6 м», «VIP Дюна 6×6 м», «VIP 12×12 м».",
    },
    {
    ...
    {
      label: "Установка",
      value: "Устанавливается на тумбы высотой 0,5 м.",
    },
  ];
