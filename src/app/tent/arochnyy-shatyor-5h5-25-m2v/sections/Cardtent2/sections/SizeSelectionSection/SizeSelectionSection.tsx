...
export const SizeSelectionSection = ({ 
  ...
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Шатры 25 м²",
      text:
        "Шатры площадью 25 кв. м — это универсальные квадратные сооружения, в которых можно устроить небольшой банкет на 15 человек, семейный праздник или пикник. При проведении фуршета возможности шатра увеличиваются — он сможет вместить до 25 человек. В компании Mobile Tent вы можете арендовать или купить арочный шатер такой вместительности по вполне приемлемой цене. Он украсит любое мероприятие и станет надежной защитой от солнца, ветра и дождя.",
    },
    {
      title: "Как устанавливать арочный шатер 5×5?",
      text:
        "Устанавливается такая конструкция на любую ровную поверхность. При желании вы можете увеличить шатер с шагом в 5 метров.",
    },
    {
      title: "Каркас и материалы",
      text:
        "Каркас такого тента выполнен из оцинкованной стальной конструкции, которая надежно удерживает купол из ПВХ.",
    },
  ];
  ...
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Высота входной арки", value: "регулируемая от 2,9 м до 4,4 м" },
    { label: "Высота в коньке", value: "регулируемая от 2,40 м до 3,9 м" },
    ...
    },
    {
      label: "Модульность",
      value:
        'Возможность увеличения площади путём соединения нескольких конструкций со стороной 5 м: "VIP 5Х5 м".',
    },
    {
