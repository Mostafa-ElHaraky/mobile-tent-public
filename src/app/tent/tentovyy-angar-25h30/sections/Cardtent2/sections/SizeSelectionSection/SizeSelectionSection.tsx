...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Тентовый ангар 25×30
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Преимущества и характеристики",
      text: `Площадь — 750 м², длина — 30 м, в коньке максимальная высота составляет 8,5 м, вес — 11,25 т.
Устойчив к ветровой (до 30 кг/п.м) и снеговой нагрузке.
Каркас выполнен из анодированного алюминия, для изготовления крепежей применяется оцинкованная сталь.`,
    },
    {
      title: "Применение",
      text: `Тентовый ангар 25×30 м может быть складом, производственным цехом, мастерской, автостоянкой, спортивным крытым сооружением.
При экономичной цене и быстрых сроках строительства эта каркасная конструкция даст те же возможности, что и капитальное сооружение, при этом прослужит до 20 лет.
Полезную площадь строения можно расширить, присоединив дополнительный ангар. По желанию клиента мы можем доукомплектовать конструкцию дополнительными опциями (отопление, освещение, пол и т.д.).`,
    },
    {
      title: "Особенности монтажа",
      text: `Выполняется без обустройства фундамента, производится крепление каркаса к поверхности и его утяжеление блоками.
Транспортировка ангара на место установки осуществляется в разобранном состоянии, при необходимости возможен его быстрый демонтаж и перевозка в другое место.
Не требуется привлечение спецтехники для монтажа.`,
    },
    {
      title: "Материалы и конструкция",
      text: `Для изготовления тентовых ангаров используются сертифицированные материалы, стойкие к коррозии и гниению.`,
    },
  ];

  // TECH SPECS — UPDATED for Тентовый ангар 25×30
  const specs: { label: string; value: string }[] = [
    { label: "Модель", value: "Тентовый ангар 25×30" },
    ...
    { label: "Вес конструкции", value: "11250 кг" },
