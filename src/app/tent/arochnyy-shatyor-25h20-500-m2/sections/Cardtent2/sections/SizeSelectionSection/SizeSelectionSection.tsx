...
export const SizeSelectionSection = ({ 
  ...
  // Default content from your code (25×20 — 500 м²)
  const defaultDescriptionBlocks: { title?: string; text: string }[] = [
    {
      title: "Описание",
      text: `Вместительный шатёр с крышей арочного типа применяется при проведении праздников, свадебных торжеств, организации презентаций и выставок. Внушительных размеров арочный шатёр размером 25×20 м и площадью 500 м² можно купить по вполне доступной цене в нашей компании.

Для проведения одного мероприятия имеется возможность недорого арендовать качественную конструкцию. Шатёр имеет форму прямоугольника и может монтироваться на любой поверхности.`,
    },
    {
      title: "Особенности модели",
      text: `Белое тентовое покрытие изготовлено из ПВХ, надёжно защищающего от непогоды, ветра и солнечных лучей.

Прочность конструкции обеспечивает выполненный из оцинкованной стали каркас, время монтажа которого составляет в среднем 12 часов.`,
    },
  ];

  // Default specs from your code (25×20 — 500 м²)
  const defaultSpecs: { label: string; value: string }[] = [
    ...
    { label: "Вес конструкции", value: "4600 кг" },
    ...
    },
    {
      label: "Модульность",
      value:
        "Возможность увеличения площади путём соединения нескольких конструкций со стороной 5 м (20×30 м, 20×35 м, 20×40 м ...) или путём присоединения нескольких конструкций со стороной 10 м — «VIP 10×10 м», «VIP-модуль Дюна 10×10 м», «VIP-модуль 20×20 м», «VIP модуль Гексагональ 20×17 м», «VIP Дополнительный модуль 10×5 м», «VIP-модуль Октогональ 24×26 м», «VIP-модуль 20×17 м LONG-1», «VIP-модуль 20×17 м LONG-2».",
    },
    {
