...
export const SizeSelectionSection = (): ReactElement => {
  // MARKETING / DESCRIPTION — UPDATED for Шестигранный шатёр Стандарт (Диаметр 15 м)
  const descriptionBlocks: { title?: string; text: string }[] = [
    {
      title:
        "Шестигранный шатёр Стандарт (Диаметр 15 м) — красивое и удобное решение",
      text: `Шестигранный шатёр «Стандарт» диаметром 15 м — просторная каркасно-тентовая конструкция площадью 176 м² для культурных, спортивных и корпоративных мероприятий. Не требует специального фундамента, быстро монтируется и демонтируется без применения спецтехники, выдерживает повторные циклы сборки/разборки и позволяет наращивать площадь путём соединения нескольких модулей.`,
    },
    {
...
• Крепёжные элементы — оцинкованная сталь DIN 50976.`,
    },
    {
      ...
      text: `• Конструкция состоит из металлического каркаса, тентового полотна, креплений, оконных и дверных проёмов.
• Мебель, полы/ковролин и электрооборудование поставляются по желанию клиента.
• Подходит для массовых культурных и спортивных событий: быстро устанавливается и демонтируется.
• Возможность увеличения площади за счёт стыковки нескольких конструкций.`,
    },
    {
      title: "Особенности монтажа",
      text: `• Устанавливается на выровненной земляной поверхности; специальный фундамент не требуется.
• Монтаж/демонтаж — без спецтехники, в минимальные сроки, без потери качества; допустимо многократное использование.
• Обязательное крепление к основанию или утяжеление бетонными блоками согласно инструкции.`,
    },
    {
      title: "О компании",
      text: `Компания специализируется на изготовлении, аренде и продаже каркасно-тентовых конструкций различного типа, а также обслуживании и ремонте шатров всех моделей. Выполняем работы по индивидуальным заказам.`,
    },
  ];

  // TECH SPECS — UPDATED for Шестигранный шатёр Стандарт (Диаметр 15 м)
  const specs: { label: string; value: string }[] = [
    { label: "Модель", value: "Шестигранный шатёр Стандарт (Диаметр 15 м)" },
