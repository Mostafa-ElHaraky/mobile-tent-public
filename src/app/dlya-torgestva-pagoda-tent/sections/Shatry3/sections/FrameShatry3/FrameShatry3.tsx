  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter only Пагода products
        const filteredProducts = products.filter((p) => p.type === 'Пагода');
        
        console.log(`Loaded ${filteredProducts.length} Пагода products`);
        
        // If no products from CMS, use fallback static data
        if (filteredProducts.length === 0) {
          console.warn('No Пагода products from CMS, using fallback data');
        ...
        console.error('Error fetching Пагода products:', error);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Пагода completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Пагода failed:', error);
      }
