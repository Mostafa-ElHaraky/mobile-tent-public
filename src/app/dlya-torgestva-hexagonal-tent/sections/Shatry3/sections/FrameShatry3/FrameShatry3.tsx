...
export const FrameShatry3 = (): ReactElement => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        console.log('Fetching Шестигранные Шатры with updated prices...');
        
        // Use the shared utility to get products with updated prices
        const products = await getProductsWithUpdatedPrices();
        
        // Filter only Шестигранные products
        const filteredProducts = products.filter((p) => p.type === 'Шестигранные');
        
        console.log(`Loaded ${filteredProducts.length} Шестигранные products`);
        
        // If no products from CMS, use fallback static data
        if (filteredProducts.length === 0) {
          console.warn('No Шестигранные products from CMS, using fallback data');
          // Filter allProducts for fallback
          const fallbackProducts = allProducts.filter((p) => p.type === 'Шестигранные');
          setArochnyeShatry(fallbackProducts);
        } else {
          setArochnyeShatry(filteredProducts);
        }
      } catch (error) {
        console.error('Error fetching Шестигранные products:', error);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (filteredProducts.length > 0) {
          setArochnyeShatry(filteredProducts);
          console.log('Scheduled price update for Шестигранные completed');
        }
      } catch (error) {
        console.error('Scheduled price update for Шестигранные failed:', error);
      }
      ...
      <section className="mx-auto max-w-[1440px] w-full relative bottom-[5px] px-4 md:px-8">
        ...
        {arochnyeShatry.length === 0 && !isLoading ? (
          <div className="text-center py-10">
            <p className="text-gray-500">Нет доступных Шестигранные шатров в данный момент</p>
          </div>
        ) : (
          <>
